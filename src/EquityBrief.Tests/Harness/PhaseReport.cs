...
internal static class PhaseReport
{
    ...
    static readonly Dictionary<string, Placement> FiguresPlaced = new(StringComparer.Ordinal)
    {
        ...
        // 12.8's worked example: three figures the code draws from the fixture and a check holds the document
        // to, and two illustrative ones that state no figure a claim rests on.
        ["Figure 11.1"] = new Placement(
            "one night's funnel drawn from the counts the swing filter stored over the committed fixture, regenerated by the check and held to the document word for word and to the fixture's gate expectation; the funnel's rule is claimed by section 15.10's funnel row",
            Check: "fixture-expectations"),
        ["Figure 11.2"] = new Placement(
            "the member passing the most gates drawn from its stored gate row, readings and closes, regenerated by the check and held to the document and to the fixture's gate expectation; each gate's rule is claimed by section 17's rows",
            Check: "fixture-expectations"),
        ["Figure 11.3"] = new Placement(
            "a replayed session's swing plan drawn resolving on the stored closes, regenerated by the check and held to the document, its outcome walked over the captured bars; the scoring rule is claimed by section 17's swing plan outcome",
            Check: "fixture-expectations"),
        ["Figure 11.4"] = new Placement(
            "the two clocks on one timeline, illustrative and labelled so, whose order section 13.8 states and whose looks section 17 claims; a picture of claims placed elsewhere and none of its own"),
        ["Figure 11.5"] = new Placement(
