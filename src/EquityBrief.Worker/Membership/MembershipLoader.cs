    ...
    string databaseFile) : IComponent
{
    ...
    // stage idempotent.
    const string Upsert = @"
        INSERT INTO membership (index_code, ticker, joined, ""left"", sector, industry, name, observed_at)
        VALUES ($index_code, $ticker, $joined, $left, $sector, $industry, $name, $observed_at)
        ON CONFLICT (index_code, ticker, IFNULL(joined, '')) DO UPDATE SET
            ...
            name = COALESCE(excluded.name, membership.name),
    ...
    // a sector for stays null. The industry is coalesced for the same reason, from 6.9, and
    // the name for the same reason again.
