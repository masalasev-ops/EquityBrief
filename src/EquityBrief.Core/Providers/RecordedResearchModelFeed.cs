...
// for byte and read by the parser the live feed uses, and priced by the live feed's own
// rates, so a replay prices a call exactly as the live pass that recorded it did.
...
public sealed class RecordedResearchModelFeed(string folder, ResearchModelSettings settings) : IResearchModelFeed
{
    ...
    public Task<ResearchAnswer> CompleteAsync(ModelRequest request, CancellationToken cancellation = default)
    {
        ...
        return File.Exists(file)
            ? Task.FromResult(DeepSeekModelFeed.Parse(File.ReadAllText(file), request.Section))
            : throw new InvalidOperationException(
                ...
                "or a mode that changed is a request nobody recorded, and it is refused here rather than answered by the " +
                "network or by a different recording.");
    }

    public decimal Price(ResearchAnswer answer) => DeepSeekModelFeed.PriceOf(settings.Model, answer);

    public decimal Ceiling(ModelRequest request) => DeepSeekModelFeed.CeilingOf(settings.Model, request);
}
