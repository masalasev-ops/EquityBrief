...
public sealed record RetryPolicy(int Attempts, TimeSpan FirstWait, TimeSpan Timeout, TimeSpan Deadline)
{
    ...
    // seconds, inside a night bounded by fifteen minutes.
    //
    ...
    // megabytes: far outside a healthy fetch and far inside the night. Fifteen
    // because it is three times the wall clock section 17 states for five
    // hundred names, so the deadline stops a night that has hung rather than one
    // that is merely slow.
    public static RetryPolicy Standard { get; } = new(
        Attempts: 3,
        FirstWait: TimeSpan.FromSeconds(2),
        Timeout: TimeSpan.FromSeconds(30),
        Deadline: TimeSpan.FromMinutes(15));
