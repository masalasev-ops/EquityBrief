...
Was:
> <p data-phase="10">The rows are ordered by how many reasons fired, most first, then by band strength, highest first, then by ticker in code-point order (see: Tonight's list orders by the reasons that fired and then band strength, which are facts about the chart). A name's band strength is the highest strength of any band the night shown stored for it, and not the strength of the band a reason fired on. <b>Not settled by research:</b> whether band strength or the plan's reward to risk breaks a tie better. <a href="#src-chung-bellotti-2021">Chung and Bellotti (2021)</a> find that levels with more prior bounces are more likely to bounce again, decaying with time, while <a href="#src-osler-2000">Osler (2000)</a> found the published strength ratings bore no consistent relation to bounces, the only significant differences negative (<a href="#open-tiebreak">which tiebreak</a>).</p>
Now:
> <p data-phase="10">The rows are ordered by how many reasons fired, most first, then by the reward to risk the night's own plan computes from its first tranche, highest first, then by ticker in code-point order (see: Tonight's list breaks a tie in fired count by the plan's reward to risk, and a row with none is drawn after every row with one and says why). A row whose plan computes none, having no tranche, no stop on its first tranche or no traded exit, is drawn after every row with one in its fired-count group and states why in the plan's own words; on the four nights 2026-09-16 to 09-21 that was 593 of the 1,463 fired rows. The ratio is read off the plan the listing kept that night, so an earlier night is ordered by its own plans. Band strength, the highest strength of any band the night stored for the name, broke the tie before this order, and each listing records it so the order it chose is the benchmark this one is measured against. <b>Not settled by research:</b> whether band strength or the plan's reward to risk breaks a tie better. <a href="#src-chung-bellotti-2021">Chung and Bellotti (2021)</a> find that levels with more prior bounces are more likely to bounce again, decaying with time, while <a href="#src-osler-2000">Osler (2000)</a> found the published strength ratings bore no consistent relation to bounces, the only significant differences negative (<a href="#open-tiebreak">which tiebreak</a>).</p>
Why: the operator ruled on 2026-09-22 that the tiebreak is the reward to risk the night's own plan computes from its first tranche, and that a row with none is drawn last in its group saying why; 10.1 builds it, so the section states the order the list is drawn in.

### 2026-09-22 - ARCHITECTURE.html - section 11.1 states what the order comparison measures and over which nights
Authorised by: A listing records the band strength the old order read, and the three orders are compared over the nights that recorded it
Was:
> <p data-phase="10">The order is measured and not assumed: three orders are compared over the twenty each would draw, the order above as the benchmark, fired count then reward to risk, and reward to risk alone, and a switch needs a comparison over both challengers that rejects at 0.05, paired by night, with the metric and the row count fixed before any record is read (see: The order tonight's list is drawn in is compared against the order it replaces, declared before any record is read). <b>Evidence:</b> <a href="#src-white-2000">White (2000)</a> on testing the best of several against a benchmark, and <a href="#src-diebold-2015">Diebold (2015)</a> on paired loss differentials with dependence-robust errors. <a href="#src-hansen-2005">Hansen (2005)</a>'s test for superior predictive ability is known here only from secondary sources.</p>
Now:
> <p data-phase="10">The order is measured and not assumed: three orders are compared over the twenty each would draw, fired count then band strength, the order this one replaced, as the benchmark, fired count then reward to risk, and reward to risk alone, and a switch needs a comparison over both challengers that rejects at 0.05, paired by night, with the metric and the row count fixed before any record is read (see: The order tonight's list is drawn in is compared against the order it replaces, declared before any record is read). The measure is the excess the candidates' test sums, block by block, over the setups among the twenty each order draws whose whole outcome window has closed, and it is read over the nights whose listings recorded the band strength, because bands and plans are dropped a year back and the listing is kept; no comparison is drawn until every order holds the floor of blocks (see: A listing records the band strength the old order read, and the three orders are compared over the nights that recorded it). <b>Judgement:</b> the strength is recorded on the listing rather than each order's draws, because the draws can be rebuilt from the fired count, the strength and the plan the row keeps. <b>Evidence:</b> <a href="#src-white-2000">White (2000)</a> on testing the best of several against a benchmark, and <a href="#src-diebold-2015">Diebold (2015)</a> on paired loss differentials with dependence-robust errors. <a href="#src-hansen-2005">Hansen (2005)</a>'s test for superior predictive ability is known here only from secondary sources.</p>
Why: the paragraph named the order above as the benchmark, which after 10.1 is the challenger, and it did not say what the comparison measures or which nights it reads, which the retention of bands and plans decides.

### 2026-09-22 - ARCHITECTURE.html - section 15.7's list orders by the plan's reward to risk and draws it on each row
Authorised by: Tonight's list breaks a tie in fired count by the plan's reward to risk, and a row with none is drawn after every row with one and says why
Was:
>   <tr><td>The list</td><td>one row per name that fired, ordered by how many fired then by band strength, at most twenty drawn. Each row: name, close, day change, trend state in a word, the distance row mark, the reasons, and beside the name a line saying so where its prices may not reflect a dividend or split</td></tr>
Now:
>   <tr><td>The list</td><td>one row per name that fired, ordered by how many fired then by the plan's reward to risk, at most twenty drawn. Each row: name, close, day change, trend state in a word, the distance row mark, the reward to risk or the plan's reason for none, the reasons, and beside the name a line saying so where its prices may not reflect a dividend or split</td></tr>
Why: the list's tiebreak is the plan's reward to risk from 10.1, and a row drawn after its neighbours has to say what put it there, so each row draws the ratio or the plan's reason for none.

### 2026-09-22 - ARCHITECTURE.html - section 15.7's note states the list's order as 10.1 draws it
Authorised by: Tonight's list breaks a tie in fired count by the plan's reward to risk, and a row with none is drawn after every row with one and says why
Was:
...
Now:
> <p class="note">The list draws each reason in a column of its own, in a set order, headed by one word with the reason's full name on it, so an evening that is one thing happening to many names reads as one stripe down one column. Each reason's record sits once at the foot of its column and inside the reason on each row, and never beside a name. The order is how many reasons fired and then the plan's reward to risk, which are facts about the chart, and a row whose plan computes no reward to risk is drawn after every row with one in its group, saying why (see: Tonight's list breaks a tie in fired count by the plan's reward to risk, and a row with none is drawn after every row with one and says why). Selecting a row draws its plan beneath the list and brings it into view (see: Selecting a row draws its plan beneath the list and is no navigation). The reason totals are tonight's counts, and a reason's record over time is the run page's (see: Tonight's reason totals are counts, and a reason's record is the run page's).</p>
Why: the note stated the order the ruling replaced and cited the decision now under Previously decided.

### 2026-09-22 - ARCHITECTURE.html - section 15.14 names the plan's reward to risk as tonight's list's tiebreak
Authorised by: Tonight's list breaks a tie in fired count by the plan's reward to risk, and a row with none is drawn after every row with one and says why
Was:
...
Now:
>   <li>No screen orders names by a judgement about the company. The universe orders by distance to a level and tonight's list by how many reasons fired and then the plan's reward to risk, each a fact about the chart. (see: Tonight's list selects the stocks worth buying or paying attention to that evening, and the improvement loop exists to make that selection better) (see: Tonight's list breaks a tie in fired count by the plan's reward to risk, and a row with none is drawn after every row with one and says why)</li>
Why: the refusal still holds, since reward to risk is a fact about the chart and not about the company, and the sentence now names the order the list is drawn in and the decision that sets it.

### 2026-09-22 - ARCHITECTURE.html - section 17's list display draws the first twenty in the list's order
Authorised by: Tonight's list breaks a tie in fired count by the plan's reward to risk, and a row with none is drawn after every row with one and says why
Was:
>   <tr><td>List display</td><td>the strongest 20 shown, the true fired count always stated (see: The page shows twenty and states the true count)</td><td>a page that shows twenty every night cannot tell you how busy the night was</td><td>app test against a fixture night of 40</td></tr>
Now:
>   <tr><td>List display</td><td>the first 20 in the list's order drawn, the true fired count always stated (see: The page shows twenty and states the true count)</td><td>a page that shows twenty every night cannot tell you how busy the night was</td><td>app test against a fixture night of 40</td></tr>
Why: the twenty drawn are the first twenty in the list's order, which from 10.1 breaks a tie by reward to risk and not by band strength, so they are not the strongest twenty.

### 2026-09-22 - BUILD_PLAN.md - phase 10's section 17 rows for the block length and the block floor land at 10.1
Authorised by: The order tonight's list is drawn in is compared against the order it replaces, declared before any record is read
Was:
> **Section 17's rows for this phase land with their constants.** `pinned-constants` fails a figure section 17 states in digits that no code constant holds, so a row written here for the block length, the block minimum, the looks, the cost or the rule version cap would fail until the checkpoint that creates the constant. Each lands at that checkpoint: the cap of 18 at 10.4, and the block length 63, the floor of 8 blocks, the looks at 8, 12 and 16 and the cost of 10 basis points at 10.2. Until then section 13.6 states each number with its derivation and what it rests on.
Now:
...
Why: 10.1's run page region counts each order's blocks of 63 sessions against the floor of 8, so the two constants exist from 10.1 and their rows land with them rather than at 10.2.

### 2026-09-22 - BUILD_PLAN.md - 10.1 reads the plan the listing kept and records the band strength on each listing
Authorised by: A listing records the band strength the old order read, and the three orders are compared over the nights that recorded it
Was:
> Tonight's list orders by how many reasons fired, then by the reward to risk the night's own plan computes from its first tranche, higher first, then by ticker. A row with no traded tranche sorts after every row with one in its fired-count group and says why, in the plan's own words. The tonight route, the name page's walk and every oracle read the plan of the night shown. A run page region measures three orders over the twenty each would draw, the old order as the benchmark (see: The order tonight's list is drawn in is compared against the order it replaces, declared before any record is read).
Now:
...
Why: bars, bands and plans are dropped a year back and the listing is kept, so the order and its benchmark are read off the listing, which has to record the band strength for the comparison to reach its two-year trigger.

### 2026-09-22 - BUILD_PLAN.md - 10.1's done condition names the recorded band strength, the two section 17 rows and the hand-worked block counts
Authorised by: A listing records the band strength the old order read, and the three orders are compared over the nights that recorded it
Was:
> **Done when** the order is read back off the page against the store in both directions, a row with no reward to risk is drawn last in its group with its reason, the fixture gains a second night replayed from its committed year of bars at the session of 2026-09-03 with the overnight queue not run for it, so no model recording is keyed on it, and both nights' orders are derived by hand in the listings expectation (owes: An earlier night's order on tonight's list asserted from the fixture), the ordering decision is superseded with its citations repointed, and the region names its benchmark and shows no comparison before its block count is reached (owes: The three orders of tonight's list compared).
Now:
...
Why: 10.1 amends its own done condition: the checkpoint writes the band strength on every listing and pins the block length and floor, and a done condition that did not name them would be narrower than what the checkpoint changes.

### 2026-09-22 - BUILD_PLAN.md - 10.2's done condition no longer lands the block length and the block floor
Authorised by: The order tonight's list is drawn in is compared against the order it replaces, declared before any record is read
Was:
> **Done when** over a constructed register and shadow column worked by hand, a sign-flip p-value agrees with a hand count of sign vectors, including a case where it and the Poisson-binomial disagree; fewer than 8 blocks are withheld; a permutation of setups returns 1; a first look at 8 blocks cannot cross at the ruled level; a look reads only closed-window setups and a nightly read never changes the verdict field; the graph passes a promoted level as stated and a retired level to no one; a same-session loss is counted with its null; the calibration's method, inputs and cap are pinned; section 17 carries the block length, the block floor, the looks and the cost with their constants; the record region states each look's power and the proposed numbers each candidate was registered with (owes: A candidate's power re-read at each look) (owes: The candidates' proposed numbers read at their looks); and no candidate's record reaches a screen beside a ticker.
Now:
...
Why: the two rows land at 10.1 with the constants its region counts by, so 10.2 lands the looks and the cost alone.

### 2026-09-22 - BUILD_PLAN.md - the earlier night's order on tonight's list, discharged
Authorised by: Tonight's list breaks a tie in fired count by the plan's reward to risk, and a row with none is drawn after every row with one and says why
Was:
> | **An earlier night's order on tonight's list asserted from the fixture** | 5.4 correction | 10.1 | the committed fixture holds listings on one night, so no expectation can show that an earlier night is ordered by that night's own bands and plans. 10.1 replays a second night from the fixture's committed year of bars, at the session of 2026-09-03 with the overnight queue not run for it so no model recording is keyed on it, and derives both nights' orders by hand in the listings expectation. Carried here from the 5.4 and 5.8 corrections of 2026-09-21, whose two-night test stores stood in for it |
Now:
...
Why: 10.1 replays the fixture's second night and derives both nights' orders by hand, which is the evidence the row waited on, so the row names what produced it.
...
### 2026-09-23 - ARCHITECTURE.html - the request drain's and the read API's catalogue rows, section 15.7's ask row and section 15.15's note say a press starts the worker's drain
Authorised by: A press writes a request and starts the worker's drain as a process of its own, and every pass waits for the off-peak hours
Was:
>     <tr><td><b>Request drain</b></td><td><span class="layer L-research">research</span></td><td>on demand, until the queue is empty</td><td>research requests, run log</td><td>research requests</td><td>the worker's <code>drain</code> verb: it takes the oldest request nobody has started, runs the <code>research</code> verb for that name, and settles the request under what the pass's own run says it came to rather than under whether the verb ran: a pass that reported unavailable is a request refused and says so. It writes no research itself (see: A request the page writes and the worker drains is what starts a pass, and the read surface writes the ask and never the research)</td></tr>

>   <tr><td><b>Read API</b></td><td><span class="layer L-serve">serve</span></td><td>on request</td><td>every store</td><td>run log, research requests</td><td>read-only access for the app; performs no computation and no fetching, and starts no process at all. A press asking for a report writes a request and a press on the queue screen takes back one nobody has started, which are the two writes it makes and the only table it writes; the research the request leads to is the worker's (see: A request the page writes and the worker drains is what starts a pass, and the read surface writes the ask and never the research)</td></tr>

> <tr><td>Ask for a report</td><td>a control on a row whose name holds none and in the selected name's region beside a line saying none is written, which writes a request rather than starting a pass</td></tr>

> <p>A request is asked for from a row on tonight's list or from a name's own page, and both write the same row, so this screen is where either is read. Nothing on it starts a pass: the worker drains what is here, and a machine that never runs leaves a request outstanding rather than losing it.</p>
Now: the request drain's row says a press starts it and it waits out a peak window before any pass; the read API's row says it starts one process, the worker's drain, from a copy of the worker's build when a press writes a request; the ask row says the control writes a request and starts the worker's drain to write it at the off-peak rate; and the queue screen's note says a press that writes a request starts the drain from a copy of the worker's build, that the drain waits for the off-peak rate, and that nothing on the screen starts one. Every citation of the superseded decision names the one replacing it.
Why: the operator ruled that a queued report is processed as soon as it is queued, with the page starting the worker on the press, and that every pass runs off-peak.

### 2026-09-23 - RUNBOOK.md - what the control does and draining the queue say a press starts the drain, from a copy, at the off-peak rate
Authorised by: A press writes a request and starts the worker's drain as a process of its own, and every pass waits for the off-peak hours
Was:
> **What the control does.** It sends the press with a header of the page's own, and the read surface refuses a request without one, so another site's page open in a browser on this machine cannot ask for a pass (see: A pass is started only by a request carrying the name page's own header). The surface refuses a name the index does not hold, then writes a request and returns at once, starting no process of its own. The pass runs when the queue is next drained, and the page shows what it wrote when it is opened again (see: A request the page writes and the worker drains is what starts a pass, and the read surface writes the ask and never the research).

> A press asks for a report and the drain writes it. Nothing on a screen starts a pass, so the queue is the one way one is asked for and this is the one way one is run:
Now: the control's paragraph says the surface writes a request, starts the worker's drain from a copy of the worker's build and returns at once, and the drain runs the pass at the off-peak rate; the drain's section opens on a press starting it and the drain run by hand taking what no press started, and gains three paragraphs: every pass waits for the off-peak rate, with the windows in UTC and in New York's time; where a press's drain runs from, being a copy under `data/drains/` named for its build and kept for a week after its last start; and that a press's drain outlives the surface, with how to find and stop one.
Why: the runbook is where the operator reads what a press does and what to do about a drain, and both changed with the code.

### 2026-09-23 - SCHEMA.md - the request store's note cites the decision that superseded the one it cited
Authorised by: A press writes a request and starts the worker's drain as a process of its own, and every pass waits for the off-peak hours
Was:
...
Now: the same note, citing the decision that replaced the one it cited. Its words stand: the read surface writes the request table and nothing a pass writes.
Why: a citation of a superseded decision is refused by `no-superseded-citation`, and what the note says about who writes the table did not change.

### 2026-09-23 - .claude/rules/checks.md - read-surface names the press starting one drain, the one place a process starts, the copy, the peak wait and the second drain
Authorised by: A press writes a request and starts the worker's drain as a process of its own, and every pass waits for the off-peak hours
Was:
> | `read-surface` | every CI run | The read API hands back every stored value unchanged, and the page draws one candle and one volume bar per stored session, matched session by session against the store; every screen is laid out in cards whose keys close on what to take from a figure, each of the seven marks is asserted over a full input and over the input it degrades on with the words it says in place of what it cannot draw, the name page opens with what it is for, its three refusals and its ten words, tonight's reasons stand in six columns in their set order, and the shell remembers the palette, defaults to the machine's and resolves an unknown route to tonight with a line naming it; every figure a reader sees on the name page is drawn at the places it is read at with the stored value whole on its element, and the names holding research are listed on their own route and found, with every other member, from the masthead's search; and the key under each figure is drawn only beside the night whose figures it explains, a key written for another night replaced by its card naming that night; and a name's listing history states what followed each evening it was listed beside the base rate its row carries and forms no rate for the name; and a screen is read at the width of the screen it is read on, the column's ceiling read off the widest picture the name page draws with the card's padding and the page's gutter around it rather than off a number kept beside the check, the column the screen's own width below it, the chart and the profile beside it each drawn at its own size with the rule that scales the pair together read, and every table the five surfaces draw read in a box of its own, named where one is not; and a name's page for an earlier night draws that night's figures and not tonight's, asserted against tonight's own page over the same store, says which evening it drew, offers no control, walks that evening's list, and answers a day the listings hold no evening on with the evening before it and a date it cannot read with tonight; and a pass the page started is watched by its own rows, the step named in the reader's words against the stages the worker writes, a pass with no row yet said to be starting and one carrying its own row said to have ended, read for the run the page was handed and never for an earlier pass, with the page's own watching read off the shell's script, the suite having no browser to run it in; and no picture the five surfaces draw is stretched to the width of what holds it, each named where one is, with nothing written inside the chart's plot, the averages and the two hues named in a row above it and the nearest band's edges drawn in its hue in the column, the panel beneath as wide as the chart's own plot and the profile beside it carrying that row's height, so a session is at one distance across the two and a price at one height; and a section a checker refused twice is named on the page by every rule that refused it, with what the checker extracted beside each rule and never the draft's own sentence, repeats collected under their rule and a reason carrying no second refusal drawn as it was stored, which rules carry a sentence being read off the shipped checker's own findings rather than listed beside the check, and the line read back off the page as well as off the reader, so the two fail apart; and every part of the page states the date it is as of on the card that holds it, with no region of its own restating them; and a name's page opens with a contents read against the regions it drew in both directions, so neither a region nobody can reach nor an entry pointing at nothing passes, numbered contiguously from where a reader starts and standing above the first region it names; and each of a name's bands states how far it sits from the stored close, counted over that name's own newest typical move and read back off the markup against a computation of the test's own, a close inside a band stating no distance rather than the gap to one of its sides; and the two cases are drawn as two labelled halves where the writer answered in the shape it was asked for, each half one of the section's own paragraphs unchanged and in the order it was written, and as the prose was written where it was not; and the risks are drawn one part to a risk where the prose says where each part ends, what would confirm a risk set beneath the risk it confirms and whatever stands before the first part opening that part, and as the prose was written where it says nowhere, with every section of a pass read back off the page for its own paragraphs joining into the prose the store holds; and each reason on tonight's list draws the values the night measured it over, read back off that reason's own panel against the store rather than off the page as a whole and asserted to sit in no attribute of it, a reason the store holds no values for saying so, with a row's record read inside its reason and the column's inside the footer's, counted apart because one is the other's prefix; and the two small controls, the one on a row of tonight's list asking for a report and the queue's taking one out, are drawn by the rules written for them, found the way a browser finds them from every rule the stylesheet states: the one asking stands off the label beside it, and every property a rule written for buttons states reaches each control's button from the control's own rule, read over the button and every element holding it in the markup each screen draws, with the reader shown to give each property to the rule that wins it; and tonight's list numbers its rows in the order they are drawn, from one, under a line above them stating how many it draws of how many fired, the footer's label spanning every column before the first reason, and the universe's paging states its count beside its own word and its page after it, no figure following another across a comma or a space alone; and the selected name's region calls what its link opens a report only where the name holds one, with the day it was written, and where it holds none says so beside the control the row carries |
Now: the same row, adding that a press writing a request asks one launcher to start one drain and one writing none starts nothing, that the shipped source starts a process in one place, the launcher, at one call, from a copy of the worker's build and never the build, found only beside the surface's own build, with an old copy removed; and that a drain waits out every peak window, worked by hand at each edge, ends without waiting where nothing is outstanding, and a second drain claims nothing the first holds.
Why: the roster is where a check states what it asserts, and 11.1's tests sit under `read-surface`.

### 2026-09-23 - ARCHITECTURE.html - the queue screen gains when each request will be written, and its note says drains run one at a time
Authorised by: The queue page states when each request will be written
Was:
> <p>A request is asked for from a row on tonight's list or from a name's own page, and both write the same row, so this screen is where either is read. A press that writes a request starts the worker's drain as a process of its own, from a copy of the worker's build, and the drain takes what is here oldest first, waiting for the off-peak rate before a pass where it starts inside a peak window or reaches one between passes. Nothing on this screen starts one, and a drain that could not be started leaves a request outstanding rather than losing it, for the next press or the drain run by hand (see: A press writes a request and starts the worker's drain as a process of its own, and every pass waits for the off-peak hours) (see: Queued work runs off-peak, and every schedule is written in UTC).</p>
Now: section 15.15 gains the row When each will be written: when the pass of every request will start or started and when it is expected to end or ended, in New York's time with the offset named and UTC beside it, now or at the end of the peak window for a request with nothing ahead of it, after the others on the median of the passes the store holds that ran to their end for one behind them, and a store holding none saying it cannot estimate. The note beneath the table adds that drains run one at a time over a store, so the order the screen states is the order passes run in.
Why: the operator asked on 2026-09-23 to know fully when a report will be generated, and the order the page states is only the order passes run in where one drain runs at a time.

### 2026-09-23 - BUILD_PLAN.md - 11.2 says drains run one at a time
Authorised by: The queue page states when each request will be written
Was:
...
Now: the same paragraph, adding that drains run one at a time over a store, one started while another runs waiting for it to end, so the order the page states is the order passes run in.
Why: every press starts a drain, so without it several could run at once and the page's order would not be the order passes run in; the checkpoint builds it, so its text names it.

### 2026-09-23 - .claude/rules/checks.md - read-surface names the queue page's times and the one drain at a time
Authorised by: The queue page states when each request will be written
Was:
> | `read-surface` | every CI run | The read API hands back every stored value unchanged, and the page draws one candle and one volume bar per stored session, matched session by session against the store; every screen is laid out in cards whose keys close on what to take from a figure, each of the seven marks is asserted over a full input and over the input it degrades on with the words it says in place of what it cannot draw, the name page opens with what it is for, its three refusals and its ten words, tonight's reasons stand in six columns in their set order, and the shell remembers the palette, defaults to the machine's and resolves an unknown route to tonight with a line naming it; every figure a reader sees on the name page is drawn at the places it is read at with the stored value whole on its element, and the names holding research are listed on their own route and found, with every other member, from the masthead's search; and the key under each figure is drawn only beside the night whose figures it explains, a key written for another night replaced by its card naming that night; and a name's listing history states what followed each evening it was listed beside the base rate its row carries and forms no rate for the name; and a screen is read at the width of the screen it is read on, the column's ceiling read off the widest picture the name page draws with the card's padding and the page's gutter around it rather than off a number kept beside the check, the column the screen's own width below it, the chart and the profile beside it each drawn at its own size with the rule that scales the pair together read, and every table the five surfaces draw read in a box of its own, named where one is not; and a name's page for an earlier night draws that night's figures and not tonight's, asserted against tonight's own page over the same store, says which evening it drew, offers no control, walks that evening's list, and answers a day the listings hold no evening on with the evening before it and a date it cannot read with tonight; and a pass the page started is watched by its own rows, the step named in the reader's words against the stages the worker writes, a pass with no row yet said to be starting and one carrying its own row said to have ended, read for the run the page was handed and never for an earlier pass, with the page's own watching read off the shell's script, the suite having no browser to run it in; and no picture the five surfaces draw is stretched to the width of what holds it, each named where one is, with nothing written inside the chart's plot, the averages and the two hues named in a row above it and the nearest band's edges drawn in its hue in the column, the panel beneath as wide as the chart's own plot and the profile beside it carrying that row's height, so a session is at one distance across the two and a price at one height; and a section a checker refused twice is named on the page by every rule that refused it, with what the checker extracted beside each rule and never the draft's own sentence, repeats collected under their rule and a reason carrying no second refusal drawn as it was stored, which rules carry a sentence being read off the shipped checker's own findings rather than listed beside the check, and the line read back off the page as well as off the reader, so the two fail apart; and every part of the page states the date it is as of on the card that holds it, with no region of its own restating them; and a name's page opens with a contents read against the regions it drew in both directions, so neither a region nobody can reach nor an entry pointing at nothing passes, numbered contiguously from where a reader starts and standing above the first region it names; and each of a name's bands states how far it sits from the stored close, counted over that name's own newest typical move and read back off the markup against a computation of the test's own, a close inside a band stating no distance rather than the gap to one of its sides; and the two cases are drawn as two labelled halves where the writer answered in the shape it was asked for, each half one of the section's own paragraphs unchanged and in the order it was written, and as the prose was written where it was not; and the risks are drawn one part to a risk where the prose says where each part ends, what would confirm a risk set beneath the risk it confirms and whatever stands before the first part opening that part, and as the prose was written where it says nowhere, with every section of a pass read back off the page for its own paragraphs joining into the prose the store holds; and each reason on tonight's list draws the values the night measured it over, read back off that reason's own panel against the store rather than off the page as a whole and asserted to sit in no attribute of it, a reason the store holds no values for saying so, with a row's record read inside its reason and the column's inside the footer's, counted apart because one is the other's prefix; and the two small controls, the one on a row of tonight's list asking for a report and the queue's taking one out, are drawn by the rules written for them, found the way a browser finds them from every rule the stylesheet states: the one asking stands off the label beside it, and every property a rule written for buttons states reaches each control's button from the control's own rule, read over the button and every element holding it in the markup each screen draws, with the reader shown to give each property to the rule that wins it; and tonight's list numbers its rows in the order they are drawn, from one, under a line above them stating how many it draws of how many fired, the footer's label spanning every column before the first reason, and the universe's paging states its count beside its own word and its page after it, no figure following another across a comma or a space alone; and the selected name's region calls what its link opens a report only where the name holds one, with the day it was written, and where it holds none says so beside the control the row carries; and a press that writes a request, from tonight's list or a name's own page, asks one launcher to start one drain and a press that writes none starts nothing, the shipped source starting a process in one place, the launcher, at one call, which starts the drain from a copy of the worker's build under the data root and never from the build itself, finds the worker's build only beside the surface's own and never beside the suite's, and removes a copy nothing has started from for longer than a copy is kept; and a drain waits for the end of every peak window it starts in or reaches between passes, worked by hand at each window's edges, ends without waiting where nothing is outstanding, and a second drain started while one runs claims nothing the first holds |
Now: the same row, adding that the queue page states for every request when its pass will start or started and will end or ended, read back off each row's cell against instants worked by hand, now, the window's end and after the requests ahead moved past a window, on the median of the finished passes stated with how many, a store holding none saying it cannot estimate, in New York's time with its offset on both sides of 2026-11-01 and UTC beside it; and that a drain started while another holds the queue waits for it to end.
Why: the roster is where a check states what it asserts, and 11.2's tests sit under `read-surface`.

### 2026-09-23 - ARCHITECTURE.html - tonight's list gains the report's state, and the ask row says the control is drawn only where none is queued
Authorised by: The queue page states when each request will be written
Was:
> <tr><td>Ask for a report</td><td>a control on a row whose name holds none and in the selected name's region beside a line saying none is written, which writes a request and starts the worker's drain to write it at the off-peak rate</td></tr>
Now: section 15.7 gains the row The report's state: on a row and in the selected name's region, what the queue holds for the name, written with the day it was written, being written since the instant its pass started, queued with the instant it will start, or not written with the control asking for one. The ask row says the control is drawn on a row whose name holds none and has none queued.
Why: a reader of tonight's list could not tell a name already asked for from one nobody had asked for, and the operator asked to know when a report will be written wherever they read the name.

### 2026-09-23 - .claude/rules/checks.md - read-surface names the report's state on a row and in the selected region
Authorised by: The queue page states when each request will be written
Was:
> | `read-surface` | every CI run | The read API hands back every stored value unchanged, and the page draws one candle and one volume bar per stored session, matched session by session against the store; every screen is laid out in cards whose keys close on what to take from a figure, each of the seven marks is asserted over a full input and over the input it degrades on with the words it says in place of what it cannot draw, the name page opens with what it is for, its three refusals and its ten words, tonight's reasons stand in six columns in their set order, and the shell remembers the palette, defaults to the machine's and resolves an unknown route to tonight with a line naming it; every figure a reader sees on the name page is drawn at the places it is read at with the stored value whole on its element, and the names holding research are listed on their own route and found, with every other member, from the masthead's search; and the key under each figure is drawn only beside the night whose figures it explains, a key written for another night replaced by its card naming that night; and a name's listing history states what followed each evening it was listed beside the base rate its row carries and forms no rate for the name; and a screen is read at the width of the screen it is read on, the column's ceiling read off the widest picture the name page draws with the card's padding and the page's gutter around it rather than off a number kept beside the check, the column the screen's own width below it, the chart and the profile beside it each drawn at its own size with the rule that scales the pair together read, and every table the five surfaces draw read in a box of its own, named where one is not; and a name's page for an earlier night draws that night's figures and not tonight's, asserted against tonight's own page over the same store, says which evening it drew, offers no control, walks that evening's list, and answers a day the listings hold no evening on with the evening before it and a date it cannot read with tonight; and a pass the page started is watched by its own rows, the step named in the reader's words against the stages the worker writes, a pass with no row yet said to be starting and one carrying its own row said to have ended, read for the run the page was handed and never for an earlier pass, with the page's own watching read off the shell's script, the suite having no browser to run it in; and no picture the five surfaces draw is stretched to the width of what holds it, each named where one is, with nothing written inside the chart's plot, the averages and the two hues named in a row above it and the nearest band's edges drawn in its hue in the column, the panel beneath as wide as the chart's own plot and the profile beside it carrying that row's height, so a session is at one distance across the two and a price at one height; and a section a checker refused twice is named on the page by every rule that refused it, with what the checker extracted beside each rule and never the draft's own sentence, repeats collected under their rule and a reason carrying no second refusal drawn as it was stored, which rules carry a sentence being read off the shipped checker's own findings rather than listed beside the check, and the line read back off the page as well as off the reader, so the two fail apart; and every part of the page states the date it is as of on the card that holds it, with no region of its own restating them; and a name's page opens with a contents read against the regions it drew in both directions, so neither a region nobody can reach nor an entry pointing at nothing passes, numbered contiguously from where a reader starts and standing above the first region it names; and each of a name's bands states how far it sits from the stored close, counted over that name's own newest typical move and read back off the markup against a computation of the test's own, a close inside a band stating no distance rather than the gap to one of its sides; and the two cases are drawn as two labelled halves where the writer answered in the shape it was asked for, each half one of the section's own paragraphs unchanged and in the order it was written, and as the prose was written where it was not; and the risks are drawn one part to a risk where the prose says where each part ends, what would confirm a risk set beneath the risk it confirms and whatever stands before the first part opening that part, and as the prose was written where it says nowhere, with every section of a pass read back off the page for its own paragraphs joining into the prose the store holds; and each reason on tonight's list draws the values the night measured it over, read back off that reason's own panel against the store rather than off the page as a whole and asserted to sit in no attribute of it, a reason the store holds no values for saying so, with a row's record read inside its reason and the column's inside the footer's, counted apart because one is the other's prefix; and the two small controls, the one on a row of tonight's list asking for a report and the queue's taking one out, are drawn by the rules written for them, found the way a browser finds them from every rule the stylesheet states: the one asking stands off the label beside it, and every property a rule written for buttons states reaches each control's button from the control's own rule, read over the button and every element holding it in the markup each screen draws, with the reader shown to give each property to the rule that wins it; and tonight's list numbers its rows in the order they are drawn, from one, under a line above them stating how many it draws of how many fired, the footer's label spanning every column before the first reason, and the universe's paging states its count beside its own word and its page after it, no figure following another across a comma or a space alone; and the selected name's region calls what its link opens a report only where the name holds one, with the day it was written, and where it holds none says so beside the control the row carries; and a press that writes a request, from tonight's list or a name's own page, asks one launcher to start one drain and a press that writes none starts nothing, the shipped source starting a process in one place, the launcher, at one call, which starts the drain from a copy of the worker's build under the data root and never from the build itself, finds the worker's build only beside the surface's own and never beside the suite's, and removes a copy nothing has started from for longer than a copy is kept; and a drain waits for the end of every peak window it starts in or reaches between passes, worked by hand at each window's edges, ends without waiting where nothing is outstanding, and a second drain started while one runs claims nothing the first holds; and the queue page states for every request when its pass will start or started and will end or ended, read back off each row's own cell against instants worked by hand: now for one with nothing ahead of it off-peak, the end of the peak window for one asked inside it, and after the requests ahead of it moved past a window it falls in, on the median of the passes the store holds that ran to their end, stated once with how many it is over, a store holding none saying it cannot estimate, each instant in New York's time with its offset on both sides of the change of 2026-11-01 and UTC beside it; and a drain started while another holds the queue waits for it to end |
Now: the same row, adding that each row of tonight's list and the selected name's region state what the queue holds for the name, written, being written since its pass started, queued with the instant it starts, or not written, read back against the store in both directions over arrangements holding each state, with the control drawn only where the name holds none and has none queued.
Why: the roster is where a check states what it asserts, and 11.3's tests sit under `read-surface`.

### 2026-09-23 - CLAUDE.md - the hard rule names the night's one request as its third carve-out
Authorised by: The night asks for a report on the first name of its list
Was:
> **The nightly run makes no model call and no per-name network request.** Bars arrive in one bulk request and news in one feed request, so a night costs the same whether the universe is fifty names or five hundred. Any component that adds a per-name call to the nightly path is a defect, not a feature. Two carve-outs are named rather than left to be discovered, and neither grows with the index: a new member's one-year backfill, once per name and again, for a name it stored nothing for, on each of the five nights after and once a week after that until one stores its year or the name leaves the index, and the corporate action refetch, which asks for a year again for a name an action landed on, on each of the five nights after a failed check and once a week after that until a refetch succeeds or the name leaves the index. (see: The nightly run is arithmetic only) (see: Adjusted history is re-fetched after a corporate action) (see: A suspect name is asked for again on the five nights after it is marked and weekly after that, and its own page, its row on tonight's list and the run page say so until a refetch succeeds) (see: A name the backfill stored nothing for is asked for again on the five nights after and weekly after that, and its page and the run page say so until one stores its year)
Now: the hard rule names three carve-outs, none growing with the index: the new member's backfill, the corporate action refetch, and the night's one request for a report on the first name drawn on its list, a paid pass the night sets off after its overnight queue whose calls and requests sit on the drain's own run and not on the night's, on the operator's ruling of 2026-09-23, citing the decision it rests on.
Why: the operator ruled on 2026-09-23, "Yes the nightly run will generate paid research for top 1 name", and the 11.0 ruling left the hard rule's words to the checkpoint whose code makes the request.

### 2026-09-23 - ARCHITECTURE.html - section 14 gains the night's own request after the queue, and the catalogue, the matrix and section 17 say what it asks for
Authorised by: The night asks for a report on the first name of its list
Was:
>     <tr><td><b>Request drain</b></td><td><span class="layer L-research">research</span></td><td>on demand, until the queue is empty</td><td>research requests, run log</td><td>research requests</td><td>the worker's <code>drain</code> verb, started by the press that writes a request and runnable by hand: it waits out a peak window before any pass, takes the oldest request nobody has started, runs the <code>research</code> verb for that name, and settles the request under what the pass's own run says it came to rather than under whether the verb ran: a pass that reported unavailable is a request refused and says so. It writes no research itself (see: A press writes a request and starts the worker's drain as a process of its own, and every pass waits for the off-peak hours)</td></tr>

> <p class="note">The arithmetic, being steps 1 to 17, calls no model and makes no per-name network request, and step 18 is carved out of the first of those by name and calls the local model only (see: The night's zero-model-call rule bounds the arithmetic, and the overnight queue is carved out of it by name). A feed that answers in pages counts every page it fetched, so a page count that grows with the index is a per-name call whatever the request count says (see: A feed that pages counts every page, and a page count that grows with the index is a per-name call). The run is idempotent in what it records about the market: running it twice on one night produces the same bars, the same membership spans and the same computed values, and where a second run computes different values than the first, because a refetch moved the prices between them, the second run's are what the store holds, the facts file included (see: A re-run replaces a night's facts file where the store now computes a different one). It does not produce a byte-identical store, and is not meant to. Each row carries the instant it was observed, and the run log gains a row per stage per run, so the store also records that the night ran twice. That is the evidence the run page reads, and a run that left no trace of having repeated would be the thing to fix rather than the thing to claim. The schedule is a UTC instant after the provider posts the day's bulk file, moved later only by a night that finds the file not yet posted, and registered with whatever scheduler the machine has, because scheduling lives outside the application and a schedule written in local time moves twice a year with nothing to announce it (see: Queued work runs off-peak, and every schedule is written in UTC) (see: The night runs at a fixed UTC instant, moved only when a night finds the day's file not yet posted) (see: Nothing is written against one operating system).</p>

>   <tr><td>Outstanding</td><td>every request nobody has started, drawn in the order the worker takes them in</td></tr>

>   <tr><td>Request drain</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td><span class="r">R</span> <span class="w">W</span></td><td><span class="w">W</span></td></tr>

>   <tr><td>Model calls in the nightly run</td><td>0 in the arithmetic, being every step before the overnight queue, with the queue, the night's last step, carved out by name and making local calls only (see: The nightly run is arithmetic only) (see: The night's zero-model-call rule bounds the arithmetic, and the overnight queue is carved out of it by name)</td><td>the whole design rests on the nightly half being free, and the carve is what keeps that true where the queue makes it false as written. The queue's calls are local and cost nothing, it runs after the arithmetic has closed and recorded its counts, and no figure above it moves whether it ran or not. Carved rather than loosened, for the reason the per-name row's own carve-outs state: a night that called a paid model for every name would satisfy a loosened rule and defeat the design</td><td>run log, on the arithmetic's stages</td></tr>

>   <tr><td>Per-name network calls in the nightly run</td><td>0 in the steady state, with the backfill and the corporate action refetch carved out of it</td><td>bars arrive in one bulk file and news in one feed request, so a night does not grow with the universe. The backfill is the one exception and is bounded rather than nightly: it makes one request per name holding no history, which is every name on the first run and a new joiner afterwards, and never again for a name that already holds its year. A name it stored nothing for is asked for again on each of the 5 nights after the first and then every 7 days until one stores its year or the name leaves the index, which is bounded by the names the provider cannot serve rather than by the universe (see: A name the backfill stored nothing for is asked for again on the five nights after and weekly after that, and its page and the run page say so until one stores its year). Written here because the limit as first stated forbade the step the run order carries at position two, and a check reading it would have failed on a rule nobody meant. The corporate action refetch is the second exception and was found at 1.6 by the same route: it makes one request per name whose adjusted prices an action moved, and a name whose refetch failed is asked for again on each of the 5 nights after the one that marked it and then every 7 days until a refetch succeeds or the name leaves the index, a failure on a night an action lands on it starting the count again, so one action costs 6 requests over the 6 nights from the one it lands on and 1 every 7 days after that however long its failure lasts. That is bounded by the actions of the day and of the 5 nights before it, and by one request every 7 days for each name whose retries are spent, rather than by the universe, and on the fixture's captured day it is one name in five hundred. A name whose retries are spent stays suspect, and its name page, its row on tonight's list and the run page say so until a refetch succeeds (see: A suspect name is asked for again on the five nights after it is marked and weekly after that, and its own page, its row on tonight's list and the run page say so until a refetch succeeds). It is carved rather than the rule loosened, because a night that refetched every name would satisfy a loosened rule and defeat the whole design</td><td>run log, on the steady-state stages</td></tr>
Now: section 14 gains a step after the overnight queue asking for a report on the first name drawn on tonight's list, and its note says the request calls no model and makes no request. The request drain's catalogue row runs once a night after the queue and reads the listings, with its matrix row reading them too. Section 17's model call row no longer calls the queue the night's last step, its per-name row names the night's request among the carve-outs, and a row counts the names the night asks for at 1. The queue screen's outstanding row names who asked.
Why: the operator ruled on 2026-09-23 that the night's first name has its report written after the run, and that the hard rule names the pass it leads to as a carve-out.

### 2026-09-23 - SCHEMA.md - the request store's insert is split between the read surface and the night's own request
Authorised by: The night asks for a report on the first name of its list
Was:
> | `research_request` | ReadApi | ReadApi, RequestDrain | none |

> **`research_request` is the one table the read surface writes, and the split is by operation.** ReadApi does two things: it inserts a request when a press asks for one, from tonight's list or from a name's page, and it updates a request nobody has claimed to `withdrawn` when a press on the queue screen takes it out. RequestDrain belongs to the worker and moves the same row through `writing` and then `written` or `refused`. No column is written by both in one operation and the declared sets are below, which is the permission `facts` is already declared under. The read surface still writes nothing a pass writes: a request is an ask, and the research it leads to is the worker's (see: A press writes a request and starts the worker's drain as a process of its own, and every pass waits for the off-peak hours).
...
Now: the ownership table gives the request store's insert to ReadApi and RequestDrain, the columns written on the insert say which writes them, `asked_from` admits `night` written by RequestDrain, the declared column sets give RequestDrain the same five on the night's request, and the note says the insert is split by what asks, a press on a screen or the night.
Why: one writer per store per operation is held by operation and by what asks, as the declared column sets already state it.

### 2026-09-23 - RUNBOOK.md - the overnight queue is no longer the night's last step
Authorised by: The night asks for a report on the first name of its list
Was:
...
Now: the queue runs after the arithmetic has closed and before the night's own request.
Why: the night's own request now runs after it.

### 2026-09-23 - .claude/rules/checks.md - nightly-run, nightly-cost and schema-columns name the night's own request
Authorised by: The night asks for a report on the first name of its list
Was:
...
> | `nightly-cost` | every CI run | The nightly path makes zero per-name network requests in its steady state and its arithmetic zero model calls, asserted over the shipped source and over a recorded run, with the run measured over two universe sizes so the count is shown not to grow with the population. The two carve-outs the hard rule names are asserted rather than exempted: a joiner's backfill is one request per name ever where it stores the name's year, and a name it stores nothing for costs one request on each of the five nights after and one a week after them, counted over a constructed name the provider serves nothing, and a suspect name costs one request on each of the five nights of its retries and one a week after them, counted over a constructed name whose refetch fails. The night reaches no lane an open reaches, read off what the components its own file constructs declare, and every model call a whole recorded night makes sits on the overnight queue's own row or on a pass that row names, with nothing spent anywhere, because the queue is carved out of the model-call rule by name and out of nothing else. A recorded night at one rule version and at the fullest register the version bound admits makes the same requests on every step and none on the version step, counted off the feeds and put on the step each was made on, with a score per version per name and the step saying how many versions of each kind it replayed, so the step's cost is shown to grow with versions and not with requests |
Now: nightly-run adds that after the queue the night asks for a report on the first name its list draws, one request and none for any other, starting the drain it was handed and recording no model call and no request on its own row, with a waiting name and a night run again for an earlier session asking for none; nightly-cost says the third carve-out is asserted under nightly-run; schema-columns adds that the request table's check on who asked admits the night, refuses any other value and keeps every row through its rebuild.
Why: the roster is where a check states what it asserts, and 11.4's tests sit under `nightly-run`.

### 2026-09-23 - ARCHITECTURE.html - the queue screen's paragraph names the night as the third asker
Corrects: 11.4 had the night ask for a report on the first name its list draws and left the queue page's key and section 15's paragraph on the queue screen naming a row on tonight's list and a name's own page as the only places a request is asked for.
Was:
> <p>A request is asked for from a row on tonight's list or from a name's own page, and both write the same row, so this screen is where either is read. A press that writes a request starts the worker's drain as a process of its own, from a copy of the worker's build, and the drain takes what is here oldest first, waiting for the off-peak rate before a pass where it starts inside a peak window or reaches one between passes. Drains run one at a time over a store, one started while another runs waiting for it to end, so the order this screen states is the order passes run in. Nothing on this screen starts one, and a drain that could not be started leaves a request outstanding rather than losing it, for the next press or the drain run by hand (see: A press writes a request and starts the worker's drain as a process of its own, and every pass waits for the off-peak hours) (see: Queued work runs off-peak, and every schedule is written in UTC).</p>
Now: a request is asked for from a row on tonight's list, from a name's own page, or by the night for the first name its list draws, and all three write the same row, so the screen is where each is read.
Why: 11.4 added the night as an asker and left this paragraph naming two, so from the first night that asks it described a screen holding requests it said nothing could write.

### 2026-09-23 - .claude/rules/checks.md - read-surface names every asker on the queue page and in its paragraph
Corrects: 11.4 had the night ask for a report on the first name its list draws and left the queue page's key and section 15's paragraph on the queue screen naming a row on tonight's list and a name's own page as the only places a request is asked for.
Was:
> | `read-surface` | every CI run | The read API hands back every stored value unchanged, and the page draws one candle and one volume bar per stored session, matched session by session against the store; every screen is laid out in cards whose keys close on what to take from a figure, each of the seven marks is asserted over a full input and over the input it degrades on with the words it says in place of what it cannot draw, the name page opens with what it is for, its three refusals and its ten words, tonight's reasons stand in six columns in their set order, and the shell remembers the palette, defaults to the machine's and resolves an unknown route to tonight with a line naming it; every figure a reader sees on the name page is drawn at the places it is read at with the stored value whole on its element, and the names holding research are listed on their own route and found, with every other member, from the masthead's search; and the key under each figure is drawn only beside the night whose figures it explains, a key written for another night replaced by its card naming that night; and a name's listing history states what followed each evening it was listed beside the base rate its row carries and forms no rate for the name; and a screen is read at the width of the screen it is read on, the column's ceiling read off the widest picture the name page draws with the card's padding and the page's gutter around it rather than off a number kept beside the check, the column the screen's own width below it, the chart and the profile beside it each drawn at its own size with the rule that scales the pair together read, and every table the five surfaces draw read in a box of its own, named where one is not; and a name's page for an earlier night draws that night's figures and not tonight's, asserted against tonight's own page over the same store, says which evening it drew, offers no control, walks that evening's list, and answers a day the listings hold no evening on with the evening before it and a date it cannot read with tonight; and a pass the page started is watched by its own rows, the step named in the reader's words against the stages the worker writes, a pass with no row yet said to be starting and one carrying its own row said to have ended, read for the run the page was handed and never for an earlier pass, with the page's own watching read off the shell's script, the suite having no browser to run it in; and no picture the five surfaces draw is stretched to the width of what holds it, each named where one is, with nothing written inside the chart's plot, the averages and the two hues named in a row above it and the nearest band's edges drawn in its hue in the column, the panel beneath as wide as the chart's own plot and the profile beside it carrying that row's height, so a session is at one distance across the two and a price at one height; and a section a checker refused twice is named on the page by every rule that refused it, with what the checker extracted beside each rule and never the draft's own sentence, repeats collected under their rule and a reason carrying no second refusal drawn as it was stored, which rules carry a sentence being read off the shipped checker's own findings rather than listed beside the check, and the line read back off the page as well as off the reader, so the two fail apart; and every part of the page states the date it is as of on the card that holds it, with no region of its own restating them; and a name's page opens with a contents read against the regions it drew in both directions, so neither a region nobody can reach nor an entry pointing at nothing passes, numbered contiguously from where a reader starts and standing above the first region it names; and each of a name's bands states how far it sits from the stored close, counted over that name's own newest typical move and read back off the markup against a computation of the test's own, a close inside a band stating no distance rather than the gap to one of its sides; and the two cases are drawn as two labelled halves where the writer answered in the shape it was asked for, each half one of the section's own paragraphs unchanged and in the order it was written, and as the prose was written where it was not; and the risks are drawn one part to a risk where the prose says where each part ends, what would confirm a risk set beneath the risk it confirms and whatever stands before the first part opening that part, and as the prose was written where it says nowhere, with every section of a pass read back off the page for its own paragraphs joining into the prose the store holds; and each reason on tonight's list draws the values the night measured it over, read back off that reason's own panel against the store rather than off the page as a whole and asserted to sit in no attribute of it, a reason the store holds no values for saying so, with a row's record read inside its reason and the column's inside the footer's, counted apart because one is the other's prefix; and the two small controls, the one on a row of tonight's list asking for a report and the queue's taking one out, are drawn by the rules written for them, found the way a browser finds them from every rule the stylesheet states: the one asking stands off the label beside it, and every property a rule written for buttons states reaches each control's button from the control's own rule, read over the button and every element holding it in the markup each screen draws, with the reader shown to give each property to the rule that wins it; and tonight's list numbers its rows in the order they are drawn, from one, under a line above them stating how many it draws of how many fired, the footer's label spanning every column before the first reason, and the universe's paging states its count beside its own word and its page after it, no figure following another across a comma or a space alone; and the selected name's region calls what its link opens a report only where the name holds one, with the day it was written, and where it holds none says so beside the control the row carries; and a press that writes a request, from tonight's list or a name's own page, asks one launcher to start one drain and a press that writes none starts nothing, the shipped source starting a process in one place, the launcher, at one call, which starts the drain from a copy of the worker's build under the data root and never from the build itself, finds the worker's build only beside the surface's own and never beside the suite's, and removes a copy nothing has started from for longer than a copy is kept; and a drain waits for the end of every peak window it starts in or reaches between passes, worked by hand at each window's edges, ends without waiting where nothing is outstanding, and a second drain started while one runs claims nothing the first holds; and the queue page states for every request when its pass will start or started and will end or ended, read back off each row's own cell against instants worked by hand: now for one with nothing ahead of it off-peak, the end of the peak window for one asked inside it, and after the requests ahead of it moved past a window it falls in, on the median of the passes the store holds that ran to their end, stated once with how many it is over, a store holding none saying it cannot estimate, each instant in New York's time with its offset on both sides of the change of 2026-11-01 and UTC beside it; and a drain started while another holds the queue waits for it to end; and each row of tonight's list and the selected name's region state what the queue holds for the name, written, being written since its pass started, queued with the instant it starts, or not written, read back off a row's own markup and off the region against the store in both directions over arrangements of the store holding each state, with the control asking for a report drawn only where the name holds none and has none queued |
Now: read-surface adds that the queue page and section 15's paragraph on it name every asker the store's check on who asked admits, read off a migrated store.
Why: the roster is where a check states what it asserts, and the correction's test sits under this row.

### 2026-09-23 - ARCHITECTURE.html - the move annotator reads the membership, section 17 carries the group floor, and the moves fixture row names each move's group
Authorised by: A large move is shown beside its group's median move over the same sessions
Was:
>   <tr><td><b>Move annotator</b></td><td><span class="layer L-compute">compute</span></td><td>nightly, per name</td><td>bar store</td><td>moves</td><td>selects the largest single-day and multi-day moves of the stored year, which become the rows of the how-it-got-here table</td></tr>

>   <tr><td>Each move beside its group</td><td>on every row of the table of the biggest moves: the median move of the name's group over the same sessions, with the group named as an industry or a sector and how many members it counted, and a key saying how to read it (see: A large move is shown beside its group's median move over the same sessions) (see: A name's group is its industry where at least five other members share it on the session, and its sector otherwise, and every surface that uses it says which and how many)</td></tr>

> <p>Four parts sit in the sections they belong to rather than in regions of their own: each move beside its group's median and the peers under the table of the biggest moves, the earnings reactions beside the earnings setups, and the dividend in the numbers. Each is display only, reaching no reason, gate, plan or candidate, and each is a card with a key saying how to read it (see: Every region is a card that states where its figures came from and how to read them). The exported report draws the four as the page does, from the same code path (see: A single report can still be exported as a self-contained file).</p>

>   <tr><td>Move annotator</td><td></td><td><span class="r">R</span></td><td></td><td><span class="w">W</span></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td><span class="w">W</span></td></tr>

>   <tr><td>moves</td><td>the largest single-day and multi-day moves of the stored year, with the span of each</td><td>section 7's move annotator</td></tr>
Now: the move annotator's catalogue row reads the membership as well as the bar store and its matrix row reads the membership; section 17 gains the row Group floor at 5, pinned to `Groups.Floor`; and section 19.1's moves row adds the group each move is read against.
Why: the annotator reads a name's group off the membership on the night's session to write each move's group median, and the floor is a constant section 17 pins from 11.5.

### 2026-09-23 - .claude/rules/checks.md - read-surface and fixture-expectations name each move's group median
Authorised by: A large move is shown beside its group's median move over the same sessions
Was:
...
> | `read-surface` | every CI run | The read API hands back every stored value unchanged, and the page draws one candle and one volume bar per stored session, matched session by session against the store; every screen is laid out in cards whose keys close on what to take from a figure, each of the seven marks is asserted over a full input and over the input it degrades on with the words it says in place of what it cannot draw, the name page opens with what it is for, its three refusals and its ten words, tonight's reasons stand in six columns in their set order, and the shell remembers the palette, defaults to the machine's and resolves an unknown route to tonight with a line naming it; every figure a reader sees on the name page is drawn at the places it is read at with the stored value whole on its element, and the names holding research are listed on their own route and found, with every other member, from the masthead's search; and the key under each figure is drawn only beside the night whose figures it explains, a key written for another night replaced by its card naming that night; and a name's listing history states what followed each evening it was listed beside the base rate its row carries and forms no rate for the name; and a screen is read at the width of the screen it is read on, the column's ceiling read off the widest picture the name page draws with the card's padding and the page's gutter around it rather than off a number kept beside the check, the column the screen's own width below it, the chart and the profile beside it each drawn at its own size with the rule that scales the pair together read, and every table the five surfaces draw read in a box of its own, named where one is not; and a name's page for an earlier night draws that night's figures and not tonight's, asserted against tonight's own page over the same store, says which evening it drew, offers no control, walks that evening's list, and answers a day the listings hold no evening on with the evening before it and a date it cannot read with tonight; and a pass the page started is watched by its own rows, the step named in the reader's words against the stages the worker writes, a pass with no row yet said to be starting and one carrying its own row said to have ended, read for the run the page was handed and never for an earlier pass, with the page's own watching read off the shell's script, the suite having no browser to run it in; and no picture the five surfaces draw is stretched to the width of what holds it, each named where one is, with nothing written inside the chart's plot, the averages and the two hues named in a row above it and the nearest band's edges drawn in its hue in the column, the panel beneath as wide as the chart's own plot and the profile beside it carrying that row's height, so a session is at one distance across the two and a price at one height; and a section a checker refused twice is named on the page by every rule that refused it, with what the checker extracted beside each rule and never the draft's own sentence, repeats collected under their rule and a reason carrying no second refusal drawn as it was stored, which rules carry a sentence being read off the shipped checker's own findings rather than listed beside the check, and the line read back off the page as well as off the reader, so the two fail apart; and every part of the page states the date it is as of on the card that holds it, with no region of its own restating them; and a name's page opens with a contents read against the regions it drew in both directions, so neither a region nobody can reach nor an entry pointing at nothing passes, numbered contiguously from where a reader starts and standing above the first region it names; and each of a name's bands states how far it sits from the stored close, counted over that name's own newest typical move and read back off the markup against a computation of the test's own, a close inside a band stating no distance rather than the gap to one of its sides; and the two cases are drawn as two labelled halves where the writer answered in the shape it was asked for, each half one of the section's own paragraphs unchanged and in the order it was written, and as the prose was written where it was not; and the risks are drawn one part to a risk where the prose says where each part ends, what would confirm a risk set beneath the risk it confirms and whatever stands before the first part opening that part, and as the prose was written where it says nowhere, with every section of a pass read back off the page for its own paragraphs joining into the prose the store holds; and each reason on tonight's list draws the values the night measured it over, read back off that reason's own panel against the store rather than off the page as a whole and asserted to sit in no attribute of it, a reason the store holds no values for saying so, with a row's record read inside its reason and the column's inside the footer's, counted apart because one is the other's prefix; and the two small controls, the one on a row of tonight's list asking for a report and the queue's taking one out, are drawn by the rules written for them, found the way a browser finds them from every rule the stylesheet states: the one asking stands off the label beside it, and every property a rule written for buttons states reaches each control's button from the control's own rule, read over the button and every element holding it in the markup each screen draws, with the reader shown to give each property to the rule that wins it; and tonight's list numbers its rows in the order they are drawn, from one, under a line above them stating how many it draws of how many fired, the footer's label spanning every column before the first reason, and the universe's paging states its count beside its own word and its page after it, no figure following another across a comma or a space alone; and the selected name's region calls what its link opens a report only where the name holds one, with the day it was written, and where it holds none says so beside the control the row carries; and a press that writes a request, from tonight's list or a name's own page, asks one launcher to start one drain and a press that writes none starts nothing, the shipped source starting a process in one place, the launcher, at one call, which starts the drain from a copy of the worker's build under the data root and never from the build itself, finds the worker's build only beside the surface's own and never beside the suite's, and removes a copy nothing has started from for longer than a copy is kept; and a drain waits for the end of every peak window it starts in or reaches between passes, worked by hand at each window's edges, ends without waiting where nothing is outstanding, and a second drain started while one runs claims nothing the first holds; and the queue page states for every request when its pass will start or started and will end or ended, read back off each row's own cell against instants worked by hand: now for one with nothing ahead of it off-peak, the end of the peak window for one asked inside it, and after the requests ahead of it moved past a window it falls in, on the median of the passes the store holds that ran to their end, stated once with how many it is over, a store holding none saying it cannot estimate, each instant in New York's time with its offset on both sides of the change of 2026-11-01 and UTC beside it; and a drain started while another holds the queue waits for it to end; and each row of tonight's list and the selected name's region state what the queue holds for the name, written, being written since its pass started, queued with the instant it starts, or not written, read back off a row's own markup and off the region against the store in both directions over arrangements of the store holding each state, with the control asking for a report drawn only where the name holds none and has none queued; and the queue page and section 15's paragraph on it name every asker the store's check on who asked admits, the check read off a migrated store so an asker it comes to admit and neither names is refused |
Now: read-surface adds that each move on a name's page is drawn beside its group's median as the annotator stored it, named with how many members it was taken over and saying so where there is none; fixture-expectations adds the group rule at the floor and one short of it, the median worked by hand, and the fixture's groups derived by hand and read off the stored moves.
Why: the roster is where a check states what it asserts, and 11.5's tests sit under these two.

### 2026-09-23 - BUILD_PLAN.md - the four parts' figures in the facts file move to 11.9, with the fixture's recordings made again
Corrects: the plan put the four parts' figures into the facts file at the checkpoints that build them, and every fixture recording is keyed on a prompt carrying that file, so each of those checkpoints would have refused every recording at once; found at 11.5 while its facts clause was being built.
Was:
...
Now: 11.5 and 11.8 lose their facts file clauses and 11.7 keeps the table's own rows without the facts file; 11.9 gains the facts file carrying each move's group median with its group and count, each print's reaction and the dividend, with every fixture recording the facts file reaches made again, the local ones on the local model and the paid ones on a spend the operator agrees before they are made.
Why: the recordings are refused as a set whenever the facts file changes, so the three parts' figures arrive together with the recordings rather than one checkpoint at a time, and the paid half waits on the operator's agreed spend.

### 2026-09-23 - ARCHITECTURE.html - the move annotator writes the peer readings, and the Peers row is read as its parts
Authorised by: Peers are shown by price alone, in section 2 beside the move table
Was:
>   <tr><td><b>Move annotator</b></td><td><span class="layer L-compute">compute</span></td><td>nightly, per name</td><td>membership, bar store</td><td>moves</td><td>selects the largest single-day and multi-day moves of the stored year, which become the rows of the how-it-got-here table</td></tr>

>   <tr><td>Peers</td><td>beneath the table of the biggest moves: every member of the name's group by price alone, in ticker order with the name's own row marked, each with its close, how far it sits below the stored year's high, its return over sixty sessions, its trend state and the distance row mark, ranking none, and a key saying how to read it (see: Peers are shown by price alone, in section 2 beside the move table)</td></tr>
Now: the move annotator's catalogue row writes the peer readings as well as the moves; section 15.9's Peers row states the table's place, its population, its order and that it ranks none, then the five columns each row carries, then its key, each part a claim of its own; section 16 gains the peer readings' store, section 17 the return window of 60 pinned to `PeerReadings.ReturnWindow`, and section 19.1 the peers expectation.
Why: the readings are the annotator's over the bars it already reads, and the row named its parts in one comma run with its citation inside the last, so a verdict over it whole would have passed with a column undrawn.

### 2026-09-23 - .claude/rules/checks.md - read-surface, fixture-expectations and gap-refusal name the peers table and its readings
Authorised by: Peers are shown by price alone, in section 2 beside the move table
Was:
...
Now: read-surface adds the peers table read back off every fixture name's page against the store; fixture-expectations adds the two readings worked by hand over constructed bars and the fixture's captured bars; gap-refusal counts six computed tables withholding a gapped name's rows, the peer readings among them, and deletes a reading a withheld name or a name holding no bars kept from an earlier night.
Why: the roster is where a check states what it asserts, and 11.6's tests sit under these three.

### 2026-09-23 - ARCHITECTURE.html - the move annotator reads the calendar and writes the earnings reaction record, and the Earnings reactions row is read as its parts
Authorised by: Each print's reaction is read from the nightly calendar and the stored bars, and reaches no reason, gate or plan
Was:
>   <tr><td><b>Move annotator</b></td><td><span class="layer L-compute">compute</span></td><td>nightly, per name</td><td>membership, bar store</td><td>moves, peer readings</td><td>selects the largest single-day and multi-day moves of the stored year, which become the rows of the how-it-got-here table, and writes each name's two readings for the peers table beside the group its moves are read against</td></tr>

>   <tr><td>Earnings reactions</td><td>beside the earnings setups: one row per print over the calendar's year behind, with its report date, its timing, the session it moved on, the estimate, the actual, the provider's surprise and that session's move, a print with no filed estimate saying none was filed, and a key saying how to read it (see: Each print's reaction is read from the nightly calendar and the stored bars, and reaches no reason, gate or plan)</td></tr>

> <p>Four parts sit in the sections they belong to rather than in regions of their own: each move beside its group's median and the peers under the table of the biggest moves, the earnings reactions beside the earnings setups, and the dividend in the numbers. Each is display only, reaching no reason, gate, plan or candidate, and each is a card with a key saying how to read it (see: Every region is a card that states where its figures came from and how to read them). A name's group is read by one rule wherever it is used (see: A name's group is its industry where at least five other members share it on the session, and its sector otherwise, and every surface that uses it says which and how many). The exported report draws the four as the page does, from the same code path (see: A single report can still be exported as a self-contained file).</p>

>   <tr><td>Move annotator</td><td><span class="r">R</span></td><td><span class="r">R</span></td><td></td><td><span class="w">W</span></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td><span class="w">W</span></td></tr>
Now: the move annotator's catalogue row reads the calendar and writes the earnings reactions, and its matrix row reads the calendar; section 15.9's Earnings reactions row states the record's place and population, then the seven things each row carries, then how a print with no estimate is drawn and its key, each part a claim of its own, with its citation moved to the note beneath the table; section 16 gains the record's store and section 19.1 the reactions expectation.
Why: the record is the annotator's over the calendar and the bars it reads, and the row named its parts in one comma run beside a citation whose own commas the reader would have taken for three more.

### 2026-09-23 - SCHEMA.md - the calendar's detail names the surprise the provider sends
Authorised by: Each print's reaction is read from the nightly calendar and the stored bars, and reaches no reason, gate or plan
Was:
> | `detail` | TEXT | JSON: what the provider carries about the event beyond its date |
Now: the calendar's `detail` note names the period, the estimate, the actual and the surprise as the provider sent them.
Why: the reaction record quotes the provider's surprise, which the calendar dropped until 11.7.

### 2026-09-23 - .claude/rules/checks.md - fixture-expectations, read-surface, gap-refusal and component-access name the earnings reaction record
Authorised by: Each print's reaction is read from the nightly calendar and the stored bars, and reaches no reason, gate or plan
Was:
...
> | `fixture-expectations` | every CI run | Each stage's serialised output over the committed fixture matches what the rules produce, with the session count derived from the trading calendar rather than frozen from a run, and every expectation file states which of the two it is; and a name's group is its industry where five other members share it and its sector one short of that, never holding the name itself, with the median of its members' moves over a move's own sessions worked by hand over an even count and a member missing a close, and the fixture's groups, each Technology name beside the other two and NFLX beside nobody, derived by hand and read off the moves the replay stored; and a name's distance below the highest high it holds and its return over sixty sessions are worked by hand over constructed bars, a series one short of the window and the close it is measured from having no return, and each fixture name's readings are worked by hand from the captured bars through the replay's own rules and read off the peer readings the replay stored, one row for every name holding bars |

> | `gap-refusal` | every CI run | A series arriving with an interior session missing is refused, that name's stored series is left as it was, and the gap's date is named on the run log; a hole at either edge is a shorter history rather than a gap, and one series alone reports that it cannot be checked rather than that it is clean, so a year the backfill asks for alone is read against the exchange's closure table and refused there, which is how a refused name asked for again on its own is refused again. Every computation over a name whose stored series already holds a hole stops, so the six computed tables withhold every row for it while the ladder and the listing still carry theirs with the gap's date as the reason, asserted per table over a constructed store rather than in one loop over the eight, and a peer reading a withheld name or a name holding no bars kept from an earlier night is deleted, because the two failures are opposite: a figure computed across the hole, and a member left without the row every member gets |

> | `read-surface` | every CI run | The read API hands back every stored value unchanged, and the page draws one candle and one volume bar per stored session, matched session by session against the store; every screen is laid out in cards whose keys close on what to take from a figure, each of the seven marks is asserted over a full input and over the input it degrades on with the words it says in place of what it cannot draw, the name page opens with what it is for, its three refusals and its ten words, tonight's reasons stand in six columns in their set order, and the shell remembers the palette, defaults to the machine's and resolves an unknown route to tonight with a line naming it; every figure a reader sees on the name page is drawn at the places it is read at with the stored value whole on its element, and the names holding research are listed on their own route and found, with every other member, from the masthead's search; and the key under each figure is drawn only beside the night whose figures it explains, a key written for another night replaced by its card naming that night; and a name's listing history states what followed each evening it was listed beside the base rate its row carries and forms no rate for the name; and a screen is read at the width of the screen it is read on, the column's ceiling read off the widest picture the name page draws with the card's padding and the page's gutter around it rather than off a number kept beside the check, the column the screen's own width below it, the chart and the profile beside it each drawn at its own size with the rule that scales the pair together read, and every table the five surfaces draw read in a box of its own, named where one is not; and a name's page for an earlier night draws that night's figures and not tonight's, asserted against tonight's own page over the same store, says which evening it drew, offers no control, walks that evening's list, and answers a day the listings hold no evening on with the evening before it and a date it cannot read with tonight; and a pass the page started is watched by its own rows, the step named in the reader's words against the stages the worker writes, a pass with no row yet said to be starting and one carrying its own row said to have ended, read for the run the page was handed and never for an earlier pass, with the page's own watching read off the shell's script, the suite having no browser to run it in; and no picture the five surfaces draw is stretched to the width of what holds it, each named where one is, with nothing written inside the chart's plot, the averages and the two hues named in a row above it and the nearest band's edges drawn in its hue in the column, the panel beneath as wide as the chart's own plot and the profile beside it carrying that row's height, so a session is at one distance across the two and a price at one height; and a section a checker refused twice is named on the page by every rule that refused it, with what the checker extracted beside each rule and never the draft's own sentence, repeats collected under their rule and a reason carrying no second refusal drawn as it was stored, which rules carry a sentence being read off the shipped checker's own findings rather than listed beside the check, and the line read back off the page as well as off the reader, so the two fail apart; and every part of the page states the date it is as of on the card that holds it, with no region of its own restating them; and a name's page opens with a contents read against the regions it drew in both directions, so neither a region nobody can reach nor an entry pointing at nothing passes, numbered contiguously from where a reader starts and standing above the first region it names; and each of a name's bands states how far it sits from the stored close, counted over that name's own newest typical move and read back off the markup against a computation of the test's own, a close inside a band stating no distance rather than the gap to one of its sides; and the two cases are drawn as two labelled halves where the writer answered in the shape it was asked for, each half one of the section's own paragraphs unchanged and in the order it was written, and as the prose was written where it was not; and the risks are drawn one part to a risk where the prose says where each part ends, what would confirm a risk set beneath the risk it confirms and whatever stands before the first part opening that part, and as the prose was written where it says nowhere, with every section of a pass read back off the page for its own paragraphs joining into the prose the store holds; and each reason on tonight's list draws the values the night measured it over, read back off that reason's own panel against the store rather than off the page as a whole and asserted to sit in no attribute of it, a reason the store holds no values for saying so, with a row's record read inside its reason and the column's inside the footer's, counted apart because one is the other's prefix; and the two small controls, the one on a row of tonight's list asking for a report and the queue's taking one out, are drawn by the rules written for them, found the way a browser finds them from every rule the stylesheet states: the one asking stands off the label beside it, and every property a rule written for buttons states reaches each control's button from the control's own rule, read over the button and every element holding it in the markup each screen draws, with the reader shown to give each property to the rule that wins it; and tonight's list numbers its rows in the order they are drawn, from one, under a line above them stating how many it draws of how many fired, the footer's label spanning every column before the first reason, and the universe's paging states its count beside its own word and its page after it, no figure following another across a comma or a space alone; and the selected name's region calls what its link opens a report only where the name holds one, with the day it was written, and where it holds none says so beside the control the row carries; and a press that writes a request, from tonight's list or a name's own page, asks one launcher to start one drain and a press that writes none starts nothing, the shipped source starting a process in one place, the launcher, at one call, which starts the drain from a copy of the worker's build under the data root and never from the build itself, finds the worker's build only beside the surface's own and never beside the suite's, and removes a copy nothing has started from for longer than a copy is kept; and a drain waits for the end of every peak window it starts in or reaches between passes, worked by hand at each window's edges, ends without waiting where nothing is outstanding, and a second drain started while one runs claims nothing the first holds; and the queue page states for every request when its pass will start or started and will end or ended, read back off each row's own cell against instants worked by hand: now for one with nothing ahead of it off-peak, the end of the peak window for one asked inside it, and after the requests ahead of it moved past a window it falls in, on the median of the passes the store holds that ran to their end, stated once with how many it is over, a store holding none saying it cannot estimate, each instant in New York's time with its offset on both sides of the change of 2026-11-01 and UTC beside it; and a drain started while another holds the queue waits for it to end; and each row of tonight's list and the selected name's region state what the queue holds for the name, written, being written since its pass started, queued with the instant it starts, or not written, read back off a row's own markup and off the region against the store in both directions over arrangements of the store holding each state, with the control asking for a report drawn only where the name holds none and has none queued; and the queue page and section 15's paragraph on it name every asker the store's check on who asked admits, the check read off a migrated store so an asker it comes to admit and neither names is refused; and each move on a name's page is drawn beside its group's median as the annotator stored it, named as an industry or a sector with how many members it was taken over, read back off every fixture name's page against the store, and a group holding nobody, or nobody holding both closes, or a move stored before the median was, says so; and a name's peers table is drawn beneath the table of the biggest moves, every member of the group its readings were taken against and the name itself in ticker order with the name's own row marked, ranking none, each row's close, trend state, distance below the year's high, return and bar count read back off every fixture name's page against the store, its distance row mark the very one the universe table draws, a return the name holds too few bars for saying not available with the count, and a group holding nobody, a name holding no readings, or a page for an earlier night saying so |
Now: fixture-expectations adds the reactions worked by hand over constructed bars and the fixture's calendar and bars; read-surface adds the record read back off every fixture name's page; gap-refusal counts seven computed tables withholding a gapped name's rows and deletes a stale reaction; component-access adds that no deciding component or pinned source reads or names the record.
Why: the roster is where a check states what it asserts, and 11.7's tests sit under these four.

### 2026-09-23 - ARCHITECTURE.html - the Dividend row is read as its parts, its citation in the note beneath the table
Authorised by: The numbers section shows the dividend the provider files, on the newest filing alone
Was:
>   <tr><td>Dividend</td><td>in the numbers section, from the newest filing alone: the forward annual rate, the forward yield as the provider states it, the payout ratio, the ex-dividend date and the pay date, or one line saying the provider files none (see: The numbers section shows the dividend the provider files, on the newest filing alone)</td></tr>

> <p>Four parts sit in the sections they belong to rather than in regions of their own: each move beside its group's median and the peers under the table of the biggest moves, the earnings reactions beside the earnings setups, and the dividend in the numbers. Each is display only, reaching no reason, gate, plan or candidate, and each is a card with a key saying how to read it (see: Every region is a card that states where its figures came from and how to read them). A name's group is read by one rule wherever it is used (see: A name's group is its industry where at least five other members share it on the session, and its sector otherwise, and every surface that uses it says which and how many). A print's reaction is read off the calendar and the stored bars, its session the one the earnings rule takes for it (see: Each print's reaction is read from the nightly calendar and the stored bars, and reaches no reason, gate or plan). The exported report draws the four as the page does, from the same code path (see: A single report can still be exported as a self-contained file).</p>
Now: section 15.9's Dividend row keeps its words and is read as its seven parts, its citation moved to the note beneath the table, where the reader would otherwise have taken the citation's opening for part of its last part.
Why: a verdict over the row whole would have passed with a value undrawn.

### 2026-09-23 - SCHEMA.md - the fundamentals payload gains the dividend part
Authorised by: The numbers section shows the dividend the provider files, on the newest filing alone
Was:
> **Two providers fill one row and `source` says which filled what, part by part.** The company financials endpoint supplies ten parts, the analysts' `ratings` among them, and the filings archive five: `segments`, `revenueTables`, `tableGrowth`, `guidance` and `facts`, which that endpoint files for no name at all. The archive's five sit on the newest filing's row alone for the reason the ratios do and one of its own: a segment table is read from one filing's report page and the guidance from one announcement's exhibit, so writing either onto a historical row would state that an older quarter's segments were this quarter's, and deriving them per filing would cost a request per row for figures nothing reads. A sixteenth part, `periodEnd`, is the quarter's end and comes from either provider on any row: the company financials endpoint labels a quarter with the last day of its month, and the archive's index states the period each 10-Q and 10-K covers, so a row takes the period of the periodic report ending within a week of that label and keeps the label only where the archive was not read or indexes no such report, and `source` names whose date it is (see: A quarter ends on the date the company's own filing states).

> `source` distinguishes three reasons a part can be empty and they are not the same morning: the provider files it for nobody, the archive answered and served none, and the archive was not read. A column that could not tell the third from the first would report a company with no segments after a failed fetch. `segments` holds the report the figures were read from, the scale the table stated, its period columns and its groups in the order the table states them; `revenueTables` holds the filing's other tables of revenue by a grouping, by market, product or region, each in that shape (see: The filing's other tables of revenue by a grouping are kept beside its segment table); `tableGrowth` holds each of those tables' groups' change on the same months a year before, computed from the columns the table states (see: A group's growth in a filing's own tables is computed from the columns the table states); `guidance` holds management's own passage with the exhibit and the date it was filed on, and never a figure struck from it (see: Guidance is stored as management's own prose, and the facts file carries each figure the passage states as the claim checker reads it); `ratings` holds the analysts' mean rating on a scale of one to five, their mean target price and how many rate the name at each of five grades from a strong buy to a strong sell, as the provider files them (see: The fundamentals row carries the analysts' ratings the provider files, on the newest filing alone); `facts` holds the archive's own filed figures under the concept each was filed against, over the same twelve quarters the filings are stored over, for a named set of concepts rather than every concept the archive holds.
Now: the company financials endpoint supplies eleven parts, the `dividend` among them, the period end is the seventeenth, and the payload note states the dividend part on the newest filing's row alone.
Why: 11.8 parses the provider's dividend object into a part of the newest filing's row.

### 2026-09-23 - .claude/rules/checks.md - read-surface names the dividend lines
Authorised by: The numbers section shows the dividend the provider files, on the newest filing alone
Was:
> | `read-surface` | every CI run | The read API hands back every stored value unchanged, and the page draws one candle and one volume bar per stored session, matched session by session against the store; every screen is laid out in cards whose keys close on what to take from a figure, each of the seven marks is asserted over a full input and over the input it degrades on with the words it says in place of what it cannot draw, the name page opens with what it is for, its three refusals and its ten words, tonight's reasons stand in six columns in their set order, and the shell remembers the palette, defaults to the machine's and resolves an unknown route to tonight with a line naming it; every figure a reader sees on the name page is drawn at the places it is read at with the stored value whole on its element, and the names holding research are listed on their own route and found, with every other member, from the masthead's search; and the key under each figure is drawn only beside the night whose figures it explains, a key written for another night replaced by its card naming that night; and a name's listing history states what followed each evening it was listed beside the base rate its row carries and forms no rate for the name; and a screen is read at the width of the screen it is read on, the column's ceiling read off the widest picture the name page draws with the card's padding and the page's gutter around it rather than off a number kept beside the check, the column the screen's own width below it, the chart and the profile beside it each drawn at its own size with the rule that scales the pair together read, and every table the five surfaces draw read in a box of its own, named where one is not; and a name's page for an earlier night draws that night's figures and not tonight's, asserted against tonight's own page over the same store, says which evening it drew, offers no control, walks that evening's list, and answers a day the listings hold no evening on with the evening before it and a date it cannot read with tonight; and a pass the page started is watched by its own rows, the step named in the reader's words against the stages the worker writes, a pass with no row yet said to be starting and one carrying its own row said to have ended, read for the run the page was handed and never for an earlier pass, with the page's own watching read off the shell's script, the suite having no browser to run it in; and no picture the five surfaces draw is stretched to the width of what holds it, each named where one is, with nothing written inside the chart's plot, the averages and the two hues named in a row above it and the nearest band's edges drawn in its hue in the column, the panel beneath as wide as the chart's own plot and the profile beside it carrying that row's height, so a session is at one distance across the two and a price at one height; and a section a checker refused twice is named on the page by every rule that refused it, with what the checker extracted beside each rule and never the draft's own sentence, repeats collected under their rule and a reason carrying no second refusal drawn as it was stored, which rules carry a sentence being read off the shipped checker's own findings rather than listed beside the check, and the line read back off the page as well as off the reader, so the two fail apart; and every part of the page states the date it is as of on the card that holds it, with no region of its own restating them; and a name's page opens with a contents read against the regions it drew in both directions, so neither a region nobody can reach nor an entry pointing at nothing passes, numbered contiguously from where a reader starts and standing above the first region it names; and each of a name's bands states how far it sits from the stored close, counted over that name's own newest typical move and read back off the markup against a computation of the test's own, a close inside a band stating no distance rather than the gap to one of its sides; and the two cases are drawn as two labelled halves where the writer answered in the shape it was asked for, each half one of the section's own paragraphs unchanged and in the order it was written, and as the prose was written where it was not; and the risks are drawn one part to a risk where the prose says where each part ends, what would confirm a risk set beneath the risk it confirms and whatever stands before the first part opening that part, and as the prose was written where it says nowhere, with every section of a pass read back off the page for its own paragraphs joining into the prose the store holds; and each reason on tonight's list draws the values the night measured it over, read back off that reason's own panel against the store rather than off the page as a whole and asserted to sit in no attribute of it, a reason the store holds no values for saying so, with a row's record read inside its reason and the column's inside the footer's, counted apart because one is the other's prefix; and the two small controls, the one on a row of tonight's list asking for a report and the queue's taking one out, are drawn by the rules written for them, found the way a browser finds them from every rule the stylesheet states: the one asking stands off the label beside it, and every property a rule written for buttons states reaches each control's button from the control's own rule, read over the button and every element holding it in the markup each screen draws, with the reader shown to give each property to the rule that wins it; and tonight's list numbers its rows in the order they are drawn, from one, under a line above them stating how many it draws of how many fired, the footer's label spanning every column before the first reason, and the universe's paging states its count beside its own word and its page after it, no figure following another across a comma or a space alone; and the selected name's region calls what its link opens a report only where the name holds one, with the day it was written, and where it holds none says so beside the control the row carries; and a press that writes a request, from tonight's list or a name's own page, asks one launcher to start one drain and a press that writes none starts nothing, the shipped source starting a process in one place, the launcher, at one call, which starts the drain from a copy of the worker's build under the data root and never from the build itself, finds the worker's build only beside the surface's own and never beside the suite's, and removes a copy nothing has started from for longer than a copy is kept; and a drain waits for the end of every peak window it starts in or reaches between passes, worked by hand at each window's edges, ends without waiting where nothing is outstanding, and a second drain started while one runs claims nothing the first holds; and the queue page states for every request when its pass will start or started and will end or ended, read back off each row's own cell against instants worked by hand: now for one with nothing ahead of it off-peak, the end of the peak window for one asked inside it, and after the requests ahead of it moved past a window it falls in, on the median of the passes the store holds that ran to their end, stated once with how many it is over, a store holding none saying it cannot estimate, each instant in New York's time with its offset on both sides of the change of 2026-11-01 and UTC beside it; and a drain started while another holds the queue waits for it to end; and each row of tonight's list and the selected name's region state what the queue holds for the name, written, being written since its pass started, queued with the instant it starts, or not written, read back off a row's own markup and off the region against the store in both directions over arrangements of the store holding each state, with the control asking for a report drawn only where the name holds none and has none queued; and the queue page and section 15's paragraph on it name every asker the store's check on who asked admits, the check read off a migrated store so an asker it comes to admit and neither names is refused; and each move on a name's page is drawn beside its group's median as the annotator stored it, named as an industry or a sector with how many members it was taken over, read back off every fixture name's page against the store, and a group holding nobody, or nobody holding both closes, or a move stored before the median was, says so; and a name's peers table is drawn beneath the table of the biggest moves, every member of the group its readings were taken against and the name itself in ticker order with the name's own row marked, ranking none, each row's close, trend state, distance below the year's high, return and bar count read back off every fixture name's page against the store, its distance row mark the very one the universe table draws, a return the name holds too few bars for saying not available with the count, and a group holding nobody, a name holding no readings, or a page for an earlier night saying so; and a name's earnings reaction record is drawn as the card after the plan's, which closes on the earnings setups, each print's report date, timing, session, estimate, actual, surprise and move read back off every fixture name's page against the store, a print with no filed estimate saying none was filed and drawing no surprise |
Now: read-surface adds the dividend drawn from the newest filing alone, a payer's five values on their elements and a non-payer's one line.
Why: the roster is where a check states what it asserts, and 11.8's page test sits under this row.

### 2026-09-24 - .claude/rules/checks.md - read-surface reads what stands before the first risk the prose numbers
Authorised by: A written section is broken into parts only where its own prose says where each part ends
Was:
> | `read-surface` | every CI run | The read API hands back every stored value unchanged, and the page draws one candle and one volume bar per stored session, matched session by session against the store; every screen is laid out in cards whose keys close on what to take from a figure, each of the seven marks is asserted over a full input and over the input it degrades on with the words it says in place of what it cannot draw, the name page opens with what it is for, its three refusals and its ten words, tonight's reasons stand in six columns in their set order, and the shell remembers the palette, defaults to the machine's and resolves an unknown route to tonight with a line naming it; every figure a reader sees on the name page is drawn at the places it is read at with the stored value whole on its element, and the names holding research are listed on their own route and found, with every other member, from the masthead's search; and the key under each figure is drawn only beside the night whose figures it explains, a key written for another night replaced by its card naming that night; and a name's listing history states what followed each evening it was listed beside the base rate its row carries and forms no rate for the name; and a screen is read at the width of the screen it is read on, the column's ceiling read off the widest picture the name page draws with the card's padding and the page's gutter around it rather than off a number kept beside the check, the column the screen's own width below it, the chart and the profile beside it each drawn at its own size with the rule that scales the pair together read, and every table the five surfaces draw read in a box of its own, named where one is not; and a name's page for an earlier night draws that night's figures and not tonight's, asserted against tonight's own page over the same store, says which evening it drew, offers no control, walks that evening's list, and answers a day the listings hold no evening on with the evening before it and a date it cannot read with tonight; and a pass the page started is watched by its own rows, the step named in the reader's words against the stages the worker writes, a pass with no row yet said to be starting and one carrying its own row said to have ended, read for the run the page was handed and never for an earlier pass, with the page's own watching read off the shell's script, the suite having no browser to run it in; and no picture the five surfaces draw is stretched to the width of what holds it, each named where one is, with nothing written inside the chart's plot, the averages and the two hues named in a row above it and the nearest band's edges drawn in its hue in the column, the panel beneath as wide as the chart's own plot and the profile beside it carrying that row's height, so a session is at one distance across the two and a price at one height; and a section a checker refused twice is named on the page by every rule that refused it, with what the checker extracted beside each rule and never the draft's own sentence, repeats collected under their rule and a reason carrying no second refusal drawn as it was stored, which rules carry a sentence being read off the shipped checker's own findings rather than listed beside the check, and the line read back off the page as well as off the reader, so the two fail apart; and every part of the page states the date it is as of on the card that holds it, with no region of its own restating them; and a name's page opens with a contents read against the regions it drew in both directions, so neither a region nobody can reach nor an entry pointing at nothing passes, numbered contiguously from where a reader starts and standing above the first region it names; and each of a name's bands states how far it sits from the stored close, counted over that name's own newest typical move and read back off the markup against a computation of the test's own, a close inside a band stating no distance rather than the gap to one of its sides; and the two cases are drawn as two labelled halves where the writer answered in the shape it was asked for, each half one of the section's own paragraphs unchanged and in the order it was written, and as the prose was written where it was not; and the risks are drawn one part to a risk where the prose says where each part ends, what would confirm a risk set beneath the risk it confirms and whatever stands before the first part opening that part, and as the prose was written where it says nowhere, with every section of a pass read back off the page for its own paragraphs joining into the prose the store holds; and each reason on tonight's list draws the values the night measured it over, read back off that reason's own panel against the store rather than off the page as a whole and asserted to sit in no attribute of it, a reason the store holds no values for saying so, with a row's record read inside its reason and the column's inside the footer's, counted apart because one is the other's prefix; and the two small controls, the one on a row of tonight's list asking for a report and the queue's taking one out, are drawn by the rules written for them, found the way a browser finds them from every rule the stylesheet states: the one asking stands off the label beside it, and every property a rule written for buttons states reaches each control's button from the control's own rule, read over the button and every element holding it in the markup each screen draws, with the reader shown to give each property to the rule that wins it; and tonight's list numbers its rows in the order they are drawn, from one, under a line above them stating how many it draws of how many fired, the footer's label spanning every column before the first reason, and the universe's paging states its count beside its own word and its page after it, no figure following another across a comma or a space alone; and the selected name's region calls what its link opens a report only where the name holds one, with the day it was written, and where it holds none says so beside the control the row carries; and a press that writes a request, from tonight's list or a name's own page, asks one launcher to start one drain and a press that writes none starts nothing, the shipped source starting a process in one place, the launcher, at one call, which starts the drain from a copy of the worker's build under the data root and never from the build itself, finds the worker's build only beside the surface's own and never beside the suite's, and removes a copy nothing has started from for longer than a copy is kept; and a drain waits for the end of every peak window it starts in or reaches between passes, worked by hand at each window's edges, ends without waiting where nothing is outstanding, and a second drain started while one runs claims nothing the first holds; and the queue page states for every request when its pass will start or started and will end or ended, read back off each row's own cell against instants worked by hand: now for one with nothing ahead of it off-peak, the end of the peak window for one asked inside it, and after the requests ahead of it moved past a window it falls in, on the median of the passes the store holds that ran to their end, stated once with how many it is over, a store holding none saying it cannot estimate, each instant in New York's time with its offset on both sides of the change of 2026-11-01 and UTC beside it; and a drain started while another holds the queue waits for it to end; and each row of tonight's list and the selected name's region state what the queue holds for the name, written, being written since its pass started, queued with the instant it starts, or not written, read back off a row's own markup and off the region against the store in both directions over arrangements of the store holding each state, with the control asking for a report drawn only where the name holds none and has none queued; and the queue page and section 15's paragraph on it name every asker the store's check on who asked admits, the check read off a migrated store so an asker it comes to admit and neither names is refused; and each move on a name's page is drawn beside its group's median as the annotator stored it, named as an industry or a sector with how many members it was taken over, read back off every fixture name's page against the store, and a group holding nobody, or nobody holding both closes, or a move stored before the median was, says so; and a name's peers table is drawn beneath the table of the biggest moves, every member of the group its readings were taken against and the name itself in ticker order with the name's own row marked, ranking none, each row's close, trend state, distance below the year's high, return and bar count read back off every fixture name's page against the store, its distance row mark the very one the universe table draws, a return the name holds too few bars for saying not available with the count, and a group holding nobody, a name holding no readings, or a page for an earlier night saying so; and a name's earnings reaction record is drawn as the card after the plan's, which closes on the earnings setups, each print's report date, timing, session, estimate, actual, surprise and move read back off every fixture name's page against the store, a print with no filed estimate saying none was filed and drawing no surprise; and the numbers section draws the dividend the provider files from the newest filing alone, a payer's forward annual rate, forward yield, payout ratio and ex-dividend and pay dates each whole on its element as the store holds them and named for the provider, and a non-payer in one line saying the provider files none |
Now: read-surface reads whatever stands before the first risk the prose numbers as opening that risk's part where the prose numbers it the first, as a part of its own where it numbers it the second, and the section as written where it numbers a later risk first.
Why: a section numbering its risks from the second states its first risk before any ordinal, and reading that as an introduction set the second risk, with what would confirm it, beneath the first one's confirmation.

### 2026-09-24 - ARCHITECTURE.html - section 11's thresholds paragraph replaced with a target, a rule and an exclusion
Corrects: the paragraph stated a method with no target and no rule, and its prediction was half right when measured: at entry zone floods as predicted, crossed a level also floods and was not predicted, and most of earnings soon's flooding was the defect corrected at 5.4 rather than the threshold. The operator found it reading the paragraph on 2026-09-23, and the eight nights from 2026-09-09 to 2026-09-18 are the measurement.
Was:
> <p><b>The thresholds in this table are proposals and are known to be untested.</b> Two of them will produce too many names as written. Earnings soon fires for a large part of the index during reporting season and goes quiet between, and at entry zone fires often because the nearest support band is rarely far away. The calibration is not a backfill: the run page records how many names fired each night and which reason contributed, so the thresholds are set against your own distribution once sixty nights of listings have accumulated, which is the quarter of trading the level window already uses and long enough that a distribution of fired counts is not one week's weather (see: Condition thresholds are calibrated from your own nights, not from a backfill) (owes: The six reason thresholds calibrated from the nights they fired on). This paragraph said "after a few weeks" until the phase 5 sign-off, against an obligation stating sixty nights, which is two places holding one fact and the vaguer of the two is the one a reader meets first. The first night at index size fired 477 of 503 on 798 reasons, and most of that was not the thresholds: earnings soon counted the stored bars after the night, which a live store never holds, so every future print read as tonight's and fired, while breakout on volume read a role set against tonight's close and could not fire at all. Both were corrected at 5.4, and the rows written before are kept as written with a line saying so where a page draws them (see: Sessions to a dated event are counted on the exchange calendar and never on stored bars) (see: Breakout on volume reads resistance at the previous session's close).</p>
Now:
> the paragraph states five things in order: that the thresholds are proposals set before any night was measured and none moved since; what calibrated means, as a target, each reason's median nightly share of the index at or below 2% and the share firing any reason at or below 6%, both proposed, with the reasoning beside them; that a night on which a reason whose own median share is below a quarter fires for more than a quarter of the index is an event session, counted, marked on the run page and left out of the sample; the rule, each threshold moved once after sixty ordinary nights to the value that puts its median share at its target, as a new decision opening a new window, and a reason no threshold brings there reported rather than forced; and that the run page shows each share against its target every night. It keeps one sentence of the old paragraph, which is a current rule and not history: rows written before the 5.4 corrections are kept as written and drawn with a line saying so wherever a page shows them.
Why: a reader of the current rule was handed a method with nothing to aim at and two pieces of history. The history is kept here instead, since it is why the 5.4 corrections exist and not part of the rule: the first night at index size fired 477 of 503 on 798 reasons, most of it not the thresholds, because earnings soon counted the stored bars after the night, which a live store never holds, so every future print read as tonight's and fired, while breakout on volume read a role set against tonight's close and could not fire at all; and the paragraph said "after a few weeks" until the phase 5 sign-off, against an obligation stating sixty nights.

### 2026-09-24 - ARCHITECTURE.html - section 13.2's condition thresholds row and the sentence beneath it name the target and the ordinary nights
Authorised by: A reason's threshold is calibrated to a target share of the index over ordinary nights and a night a usually quiet reason floods is left out
Was:
> <tr><td><b>Condition thresholds</b></td><td>how close to a band counts, what volume multiple counts. Adjusted against the recorded distribution of fired counts. Changes how long the list is more than how good it is</td><td>none. Phase 5 is what first records how many names fire each night, and the calibration itself waits on 60 nights of that record, which no checkpoint accumulates. It is carried as an operating obligation and read on the run page, using those nights rather than a backfill (owes: The six reason thresholds calibrated from the nights they fired on)</td></tr>

> <p>Only the first is cheap enough to do without resolved setups, and it is the one that matters least: it needs sixty nights of fired counts, where the other two need setups that have reached a target or a stop. All three wait on the calendar rather than on a checkpoint, which is why the other two sit in phase 8 and why phase 5 has an obligation described below. A fourth row stood here until 8.0, the model's own proposal stored beside the computed plan and scored with it, and it is dropped rather than carved out of the rule that no model is asked for a number (see: The model writes no entry or exit proposal, and the loop scores only what code computes).</p>
Now:
> the row says each threshold is moved once to the value that puts its median nightly share of the index at its target over the ordinary nights recorded, and that the calibration waits on 60 ordinary nights; the sentence beneath says sixty ordinary nights.
Why: the row stated the method section 11 no longer states, and the obligation it cites now opens on ordinary nights, which `architecture-conformance` holds the row to word for word.

### 2026-09-24 - ARCHITECTURE.html - the facts assembler's catalogue row reads the reaction record and carries the phase 11 parts' figures
Authorised by: Every number in written prose must exist in the facts file
Was:
> <tr><td><b>Facts assembler</b></td><td><span class="layer L-compute">compute</span></td><td>nightly, per name, and for its night again when an open fetches a name's fundamentals</td><td>membership, bar store, indicators, swings, volume profile, levels, ladders, moves, calendar, fundamentals</td><td>facts</td><td>writes tonight's facts file with every number and its source, and its hash. The fundamentals joined its reads at 6.1, which is the checkpoint that created that store, and the newest filing a name holds gives the quarter's figures, its growth and its earnings against the estimate, the balance sheet, the valuation, the filing's own tables with the same months a year before and each group's growth, and every figure management's located guidance states (see: The facts file carries a quarter's growth, its earnings against the estimate and the filing's own tables with their year-earlier columns) (see: Guidance is stored as management's own prose, and the facts file carries each figure the passage states as the claim checker reads it). The membership row gives the company's name (see: The membership row carries the company's name the index feed states). The listings went to the change detector at 5.4 rather than here, because the retention that needed them is an update to a row this component inserts</td></tr>
Now:
> the reads gain the earnings reactions, and the description says that from 11.9 the name's group and each move's median with how many members it counted, each print's reaction newest first and the newest filing's dividend are carried, so a section may quote what the page draws beside them.
Why: 11.9 puts those figures in the facts file, which the component's declaration now states, and `component-access` reconciles the row against it.

### 2026-09-24 - BUILD_PLAN.md - 11.9 carries the operator's addendum, and the six-thresholds obligation is told its target and its exclusion
Authorised by: A reason's threshold is calibrated to a target share of the index over ordinary nights and a night a usually quiet reason floods is left out
Was:
> **Done when** the phase report is green with no claim unexamined and the predicted pair is checked against the actual; the facts file carries the medians, the reactions and the dividend, asserted over the fixture; and every fixture recording the facts file reaches is made again, with the suite green over them.
...
Now:
> 11.9's text gains a paragraph naming the addendum and its done condition gains the run page's region, read back off the page over constructed nights, and section 17's two targets and event share held by their constants. The obligation's trigger is 60 ordinary nights, 11.9's region counts each reason's ordinary nights against the sixty, and the row states the target, 2% for each reason and 6% for any reason, both proposed, and the event-session exclusion.
Why: the operator issued the addendum as part of 11.9 on 2026-09-23, and asked that the calibration be told both the target and the exclusion rather than rediscover them.

### 2026-09-24 - .claude/rules/checks.md - the read-surface roster row names the run page's region stating each reason's share against its target
Authorised by: A reason's threshold is calibrated to a target share of the index over ordinary nights and a night a usually quiet reason floods is left out
Was:
> | `read-surface` | every CI run | The read API hands back every stored value unchanged, and the page draws one candle and one volume bar per stored session, matched session by session against the store; every screen is laid out in cards whose keys close on what to take from a figure, each of the seven marks is asserted over a full input and over the input it degrades on with the words it says in place of what it cannot draw, the name page opens with what it is for, its three refusals and its ten words, tonight's reasons stand in six columns in their set order, and the shell remembers the palette, defaults to the machine's and resolves an unknown route to tonight with a line naming it; every figure a reader sees on the name page is drawn at the places it is read at with the stored value whole on its element, and the names holding research are listed on their own route and found, with every other member, from the masthead's search; and the key under each figure is drawn only beside the night whose figures it explains, a key written for another night replaced by its card naming that night; and a name's listing history states what followed each evening it was listed beside the base rate its row carries and forms no rate for the name; and a screen is read at the width of the screen it is read on, the column's ceiling read off the widest picture the name page draws with the card's padding and the page's gutter around it rather than off a number kept beside the check, the column the screen's own width below it, the chart and the profile beside it each drawn at its own size with the rule that scales the pair together read, and every table the five surfaces draw read in a box of its own, named where one is not; and a name's page for an earlier night draws that night's figures and not tonight's, asserted against tonight's own page over the same store, says which evening it drew, offers no control, walks that evening's list, and answers a day the listings hold no evening on with the evening before it and a date it cannot read with tonight; and a pass the page started is watched by its own rows, the step named in the reader's words against the stages the worker writes, a pass with no row yet said to be starting and one carrying its own row said to have ended, read for the run the page was handed and never for an earlier pass, with the page's own watching read off the shell's script, the suite having no browser to run it in; and no picture the five surfaces draw is stretched to the width of what holds it, each named where one is, with nothing written inside the chart's plot, the averages and the two hues named in a row above it and the nearest band's edges drawn in its hue in the column, the panel beneath as wide as the chart's own plot and the profile beside it carrying that row's height, so a session is at one distance across the two and a price at one height; and a section a checker refused twice is named on the page by every rule that refused it, with what the checker extracted beside each rule and never the draft's own sentence, repeats collected under their rule and a reason carrying no second refusal drawn as it was stored, which rules carry a sentence being read off the shipped checker's own findings rather than listed beside the check, and the line read back off the page as well as off the reader, so the two fail apart; and every part of the page states the date it is as of on the card that holds it, with no region of its own restating them; and a name's page opens with a contents read against the regions it drew in both directions, so neither a region nobody can reach nor an entry pointing at nothing passes, numbered contiguously from where a reader starts and standing above the first region it names; and each of a name's bands states how far it sits from the stored close, counted over that name's own newest typical move and read back off the markup against a computation of the test's own, a close inside a band stating no distance rather than the gap to one of its sides; and the two cases are drawn as two labelled halves where the writer answered in the shape it was asked for, each half one of the section's own paragraphs unchanged and in the order it was written, and as the prose was written where it was not; and the risks are drawn one part to a risk where the prose says where each part ends, what would confirm a risk set beneath the risk it confirms, whatever stands before the first risk the prose numbers opening that risk's part where it is numbered the first and standing as a part of its own where it is numbered the second, and as the prose was written where it says nowhere or numbers a later risk first, with every section of a pass read back off the page for its own paragraphs joining into the prose the store holds; and each reason on tonight's list draws the values the night measured it over, read back off that reason's own panel against the store rather than off the page as a whole and asserted to sit in no attribute of it, a reason the store holds no values for saying so, with a row's record read inside its reason and the column's inside the footer's, counted apart because one is the other's prefix; and the two small controls, the one on a row of tonight's list asking for a report and the queue's taking one out, are drawn by the rules written for them, found the way a browser finds them from every rule the stylesheet states: the one asking stands off the label beside it, and every property a rule written for buttons states reaches each control's button from the control's own rule, read over the button and every element holding it in the markup each screen draws, with the reader shown to give each property to the rule that wins it; and tonight's list numbers its rows in the order they are drawn, from one, under a line above them stating how many it draws of how many fired, the footer's label spanning every column before the first reason, and the universe's paging states its count beside its own word and its page after it, no figure following another across a comma or a space alone; and the selected name's region calls what its link opens a report only where the name holds one, with the day it was written, and where it holds none says so beside the control the row carries; and a press that writes a request, from tonight's list or a name's own page, asks one launcher to start one drain and a press that writes none starts nothing, the shipped source starting a process in one place, the launcher, at one call, which starts the drain from a copy of the worker's build under the data root and never from the build itself, finds the worker's build only beside the surface's own and never beside the suite's, and removes a copy nothing has started from for longer than a copy is kept; and a drain waits for the end of every peak window it starts in or reaches between passes, worked by hand at each window's edges, ends without waiting where nothing is outstanding, and a second drain started while one runs claims nothing the first holds; and the queue page states for every request when its pass will start or started and will end or ended, read back off each row's own cell against instants worked by hand: now for one with nothing ahead of it off-peak, the end of the peak window for one asked inside it, and after the requests ahead of it moved past a window it falls in, on the median of the passes the store holds that ran to their end, stated once with how many it is over, a store holding none saying it cannot estimate, each instant in New York's time with its offset on both sides of the change of 2026-11-01 and UTC beside it; and a drain started while another holds the queue waits for it to end; and each row of tonight's list and the selected name's region state what the queue holds for the name, written, being written since its pass started, queued with the instant it starts, or not written, read back off a row's own markup and off the region against the store in both directions over arrangements of the store holding each state, with the control asking for a report drawn only where the name holds none and has none queued; and the queue page and section 15's paragraph on it name every asker the store's check on who asked admits, the check read off a migrated store so an asker it comes to admit and neither names is refused; and each move on a name's page is drawn beside its group's median as the annotator stored it, named as an industry or a sector with how many members it was taken over, read back off every fixture name's page against the store, and a group holding nobody, or nobody holding both closes, or a move stored before the median was, says so; and a name's peers table is drawn beneath the table of the biggest moves, every member of the group its readings were taken against and the name itself in ticker order with the name's own row marked, ranking none, each row's close, trend state, distance below the year's high, return and bar count read back off every fixture name's page against the store, its distance row mark the very one the universe table draws, a return the name holds too few bars for saying not available with the count, and a group holding nobody, a name holding no readings, or a page for an earlier night saying so; and a name's earnings reaction record is drawn as the card after the plan's, which closes on the earnings setups, each print's report date, timing, session, estimate, actual, surprise and move read back off every fixture name's page against the store, a print with no filed estimate saying none was filed and drawing no surprise; and the numbers section draws the dividend the provider files from the newest filing alone, a payer's forward annual rate, forward yield, payout ratio and ex-dividend and pay dates each whole on its element as the store holds them and named for the provider, and a non-payer in one line saying the provider files none |
Now:
...
### 2026-09-24 - .claude/rules/checks.md - the fixture-expectations roster row names the facts file carrying the phase 11 parts' figures
Authorised by: Every number in written prose must exist in the facts file
Was:
> | `fixture-expectations` | every CI run | Each stage's serialised output over the committed fixture matches what the rules produce, with the session count derived from the trading calendar rather than frozen from a run, and every expectation file states which of the two it is; and a name's group is its industry where five other members share it and its sector one short of that, never holding the name itself, with the median of its members' moves over a move's own sessions worked by hand over an even count and a member missing a close, and the fixture's groups, each Technology name beside the other two and NFLX beside nobody, derived by hand and read off the moves the replay stored; and a name's distance below the highest high it holds and its return over sixty sessions are worked by hand over constructed bars, a series one short of the window and the close it is measured from having no return, and each fixture name's readings are worked by hand from the captured bars through the replay's own rules and read off the peer readings the replay stored, one row for every name holding bars; and each print's earnings reaction is worked by hand over constructed bars for each of the three timings, its session the one the earnings rule takes, a print with no estimate keeping its actual and carrying no surprise, and a print the bars do not reach or with no stored close before its session left out and counted, and the fixture's sixteen prints are worked by hand from the captured calendar and bars through the replay's own rules and read off the reactions the replay stored, one row for every print the bars reach |
Now:
> the row as it stood, with a closing clause naming what the check now asserts.
Why: a roster row names what its check asserts, and 11.9 added that assertion.

### 2026-09-24 - .claude/rules/checks.md - the obligation-reconciles roster row states each phase's count of operating rows in advance
Corrects: the check read the newest report entry against a floor of two operating rows, which refuses the report of a phase that opened none, as phase 11 did, rather than reading it; found writing 11.9's report entry.
Was:
