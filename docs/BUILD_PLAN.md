...
|---|---|---|---|
| **Architecture cites its decisions by name at each rule** | 0.5 | 3.0 | 3.0 reads the remaining sections and decides, decision by decision, whether the document states a rule that rests on each. Discharged at 1.8 for the rules phase 1 reaches |
...
| **The weighted-call stop asserted where `nightly-cost` reaches it** | 2.7 sign-off | 3.1 | 3.1 constructs a night at the allowance, which is what makes the stop observable rather than read |
| **The membership uniqueness asserted by a permanent test** | 2.7 sign-off | 3.1 | 3.1 puts the three refusals and the control against a store the shipped runner migrated |
| **The decisions the architecture states no rule about, named as such** | 3.0 | 3.0 | 3.0's reading is what says whether the residue is decisions the document states no rule about, which closes it, or a reading left unfinished, which names what finishes it |
