...
| **The swing boundaries the committed fixture cannot reach** | 3.7 sign-off | 4.4 | 4.4 writes the constructed-input tests over the level arithmetic, and the swing cases land with them rather than one checkpoint writing one of the two sets. It stood at 4.1, where the trend classifier reads swings and reaches neither case: the plateau rule and the outside day that is a peak and a trough at once are unreachable from four names of committed bars and from a classifier that only asks which swing is later |
