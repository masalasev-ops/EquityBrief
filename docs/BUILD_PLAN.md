...
| `AbsolutePaths.LooksAbsolute` reads only the first two characters | 0.7 review | 1.3 | `store-portability` sees an absolute path only where a value starts with one. `run_log.detail` is where exception text lands and exception text carries absolute paths mid-string, so the first rows that could carry one are the first rows a night writes |
| `FixtureManifest.IsUtcInstant` resolves a zoneless instant against the machine zone | 0.7 review | 1.3 | the same manifest passes on a UTC runner and fails on the operator's machine, and the existing test uses an explicit offset so it does not cover the case. It is also an implicit read of the machine zone that `clock-usage` does not match, to be widened in the same pass |
| The manifest checker scans `input.query` only | 0.7 review | 1.3 | `fixtures/manifest.schema.json` and `fixtures/README.md` both say no credential appears in a captured response and that the check scans as well, and it never opens the response and never checks that the file named by `input.file` exists |
| The money column list of `price-storage-form` is hand maintained | 0.7 review | 1.2 | nothing reconciles it against `SCHEMA.md`, which `StoreSchema.Declared` already parses, so a money column added under a new name is unchecked and nothing says so |
| `Shell.Run` reads standard output to completion before standard error | 0.7 review | 1.2 | a child that fills the error pipe's buffer deadlocks both. Latent at the output sizes the suite's children produce today, and the suite gains children that produce more |
| The runtime money-precision property has no guard | 0.7 review | 1.2 | `STRICT` was disproved at 0.2 and nothing replaced it, so the rule rests on `price-storage-form` reading migration text alone. Discharged by a repository guard that refuses a non-decimal into a money column and a test that inserts a double and asserts the refusal |
