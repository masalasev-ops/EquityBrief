...
| **Section 10's figure rows placed as claims or the corpus stating why they are not** | 4.9 sign-off | 5.0 | a placement question rather than a test. The 203 claims come from sections 7, 14, 15, 16, 17, 18 and 19.1 and the read and write matrix, so figure 10.1's rows are reached by nothing, which is the class that produced the trailing stop drift |
