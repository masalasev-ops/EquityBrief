...
Adds:       what the sign-off should read first, written into the record rather than left in a
            session's conversation. A handover that lives only in a transcript is a hole in the
            record, and the session that signs this phase off is by rule not the session that
            wrote it.
State:      both gates re-run on `main` after the last merge rather than reported from the
            branch. `tools/ci` green end to end, all 6 steps, 380 tests. `tools/verify-phase`
            green: 186 claims, 64 pass, 0 fail, 0 unexamined, 122 out of scope, 70 placements
            and verdicts reconciled against a floor of 34, 30 checks ran with none failed and
            none unrun, fixture at 6 constituents and 4 names. All eight phase 3 checkpoints
            are in the plan and all eight are recorded above. No open pull request, no
            unmerged branch, and no open obligation due inside phase 3: the table holds 27
            rows, 17 discharged, and the earliest of the 10 open ones is due at 4.0.
Who:        this session committed code at 3.2, 3.3, 3.4, 3.5 and 3.6 and may not sign any of
            it off. The sign-off is owed on the phase as a whole, by a fresh session, before
            phase 4's plan, and it does not gate anything already merged.
First:      the tautology pattern, and it is the highest value thing to sweep for. Twice in two
            consecutive checkpoints a test named for a property asserted something true by
            construction and stayed green under a mutation that removed the property. At 3.3
            the test named for the volume spreading asserted the expectation's stated overlaps
            against the stored band edges, and deleting the spreading outright left it green.
            At 3.4 the test named for touches not widening a band asserted the edges against
            the anchors, and widening the band to all its members left it green. Both are
            repaired in their own checkpoints by a test that computes the answer by hand over
            constructed input, so no hole stands open. What is not established is whether the
            other phase 3 tests carry the same shape, and both instances were found by mutation
            and neither by reading, which says how to look.
Second:     section 12's retention row against six ownership rows, which is contradiction A and
            contradiction H a third time and unnoticed until this pass. Section 12 states
            "Indicators, swings, volume profile, levels, ladders, moves: one year, recomputed
            nightly and kept for the harness", and `SCHEMA.md` gives Delete to nobody for any
            of the six. A and H were exactly this shape, a retention window with no declared
            deleter, and both were resolved at 1.4 by declaring one.
            It fails no done condition and breaks no check, so it does not reopen the phase:
            the claim is out of scope until 5.2 and nothing stored today is wrong. Two things
            make it worth a ruling rather than a note. Its due point is 5.2, which is where the
            last of the six tables arrives and not where a deleter would be built, so it may be
            a deferral naming a point that produces no evidence, which is the defect 3.0 swept
            for. And the two tables keyed on an as-of date, `volume_profile` and `level`, write
            a new set every night rather than replacing one, so they are where unbounded growth
            actually bites: twenty bands a name a night for the profile alone. `indicator` and
            `swing` key on the session the value belongs to and replace, so they grow only with
            the series.
Third:      the strength score is dominated by touches, which is the literal reading of figure
            9.1 and was recorded at 3.4 as a consequence to watch rather than as a defect.
            AAPL's immediate support band scores 57, of which 54 are members and 39 of those
            are touches, so the widest band is the strongest by construction. It now has four
            names of different character to be judged against rather than the one it was
            written on.
Fourth:     the level builder's as-of swing filter is unexercised through the builder. While
            the as-of date is a name's own last session the filter can exclude nothing, because
            a swing exists only where three sessions follow it. It reads through
            `StoredSwings.AsOf` rather than through a second copy of the clause, so 3.2's test
            is what fails on the rule, but nothing exercises it on the path the level builder
            takes.
Fifth:      the claim prediction's failure shape, which 3.7 records and which 4.0 has to apply
            rather than repeat. The prediction counted what the phase adds to the document and
            missed what the phase makes assertable in rows the document already had, and all
            eight it missed predate phase 3.
Swept:      one thing measured and clean, stated so the sign-off does not spend time on it. The
            price rounding defect 3.6 found in the volume profile was the last of its kind:
            every rounding of a price in the shipped source now goes through `PriceForm`, and
            the only occurrences of `decimal.Round` or `Math.Round` outside the tests are the
            one inside `PriceForm` itself and a sentence in a comment naming it.
Not:        the three obligations carried to 4.0 are carried and cited, not open holes, and the
            sign-off is not owed a ruling on them. They are section 19.1's fixture table
            reconciled against the expectations that exist, the claims still due at a phase
            rather than at a checkpoint, and the suite's fixture populations read from the
            expectation rather than written into each test.
